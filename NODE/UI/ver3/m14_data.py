        ...
        for date_str, group in self.data.groupby('DATE'):
            ...
            if os.path.exists(data_file):
                try:
                    ...
                    existing_data['CURRTIME'] = existing_data['CURRTIME'].astype(str)
                    # 기존 + 새 데이터 병합 후 중복 제거
                    merged_data = pd.concat([existing_data, group_save], ignore_index=True)
                    merged_data = merged_data.drop_duplicates(subset=['CURRTIME'], keep='last')
                    merged_data = merged_data.sort_values('CURRTIME').reset_index(drop=True)
                    merged_data.to_csv(data_file, index=False)
                except Exception as e:
                    print(f"[M14] 데이터 병합 실패, 덮어쓰기: {e}")
            ...
            if os.path.exists(pred_file):
                try:
                    ...
                    existing_pred['CURRTIME'] = existing_pred['CURRTIME'].astype(str)
                    merged_pred = pd.concat([existing_pred, new_pred_df], ignore_index=True)
                    merged_pred = merged_pred.drop_duplicates(subset=['CURRTIME'], keep='last')
                    merged_pred = merged_pred.sort_values('CURRTIME').reset_index(drop=True)
                    merged_pred.to_csv(pred_file, index=False)
                except Exception as e:
                    print(f"[M14] 예측 병합 실패, 덮어쓰기: {e}")
                    new_pred_df.to_csv(pred_file, index=False)
            else:
                new_pred_df.to_csv(pred_file, index=False)
