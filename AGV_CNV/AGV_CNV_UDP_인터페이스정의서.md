...
| 11 | (빈 값 121건) / 정수값 (34건, 범위 `445`~`25591`) | F8 알람 발생 시에만 채워지는 경향 |
...
## 5. 코드 정의(참조용) vs 실측 wire 포맷 비교

`AgvMsgWorkerRunnable.VHL_STATE_REPORT` 의 인덱스 정의(코드 기준)와 본 데이터 파일의 wire 포맷 차이만 사실로 기재.
