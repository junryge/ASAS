 ...
 * - org.quartz.Job, JobExecutionContext, JobExecutionException
 * - java.util.*, concurrent.*
 * - DataService, DataSet, Util, Env (from framework)
 * - RailEdge, RAIL_EDGE_TRAFFIC (from framework)
 * - Tuple (from framework)
 * - FunctionItem (from framework)
 * - LayoutUtil, TibrvService, OhtMsgWorkerRunnable (from framework)
 * - XmlUtil (from framework)
 */

/**
 * AGV traffic batch job for 1-minute interval collection of rail edge velocity data.
 * Adapted from TrafficBatch to handle AGV-specific traffic metrics.
 * Collects velocity, density, vehicle count, and passage count data from RailEdges.
 * Batch-inserts aggregated data into Logpresso "ATLAS_RAIL_TRAFFIC" table.
 * Supports M14A-specific center area velocity calculation.
 */
public class AgvTrafficBatch implements Job{
		...
		if (Util.isCurrentIC()) {
			...
			for (Map.Entry<String, FunctionItem> functionItemEntry : Env.getSwitchMap().entrySet()) {
				...
				if (functionItem != null && functionItem.isUseRailTraffic()) {
					...
					logger.info("... `AgvTrafficBatch` has started [fab: {} | mcp: {}]", fabId, mcpName);
					...
					if (checkTimer >= DELAYED_TIME) {
						logger.error("... !!!DELAYED!!! `AgvTrafficBatch` has finished [fab: {} | mcp: {}] [elapsed time: {}m ({}ms)]", fabId, mcpName, checkTimer / (60 * 1000), checkTimer);
					} else {
						logger.info("... `AgvTrafficBatch` has finished [fab: {} | mcp: {}] [elapsed time: {}ms]", fabId, mcpName, checkTimer);
					}
				}
			}
		}
	...
	// run 진행 전 선행 처리(전처리) --- 현재는 M14A 에 한정
...
//		}
	 ...
	 * @param specificallyVelocity 송신할 특정 레일의 평균 속력 값 --- M14A(center) 평균 속력
	 */
