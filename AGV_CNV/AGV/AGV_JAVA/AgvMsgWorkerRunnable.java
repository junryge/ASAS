 ...
 * - java.util.Date
 * - DataService, DataSet (from framework)
 * - Vhl and related enums (VHL_STATE, VHL_DET_STATE, RUN_CYCLE, VHL_CYCLE)
 * - RailEdge, AbstractEdge, EDGE_TYPE
 * - RailNode, DijkstraVhlRouteFind
 * - LogpressoAPI, Tuple
 * - Util, Env, LayoutUtil, Navigator, FabProperties, FunctionItem, FunctionType
 * - Record items (HidOffRecordItem, VhlOffRecordItem, StageCommandRecordItem)
 * - TibrvService
 */

/**
 * AGV message worker runnable for processing vehicle state reports (Text ID:2).
 * Adapted from OhtMsgWorkerRunnable to handle AGV-specific messages.
 * Handles CLW MCP7 control and processes vehicle state transitions with rail velocity
 * calculations, route finding via Dijkstra algorithm, and Logpresso data storage.
 * Supports HID OFF and VHL OFF alarm processing with TIB/RV messaging.
 */
public class AgvMsgWorkerRunnable implements Runnable {
    private static final Logger logger = LoggerFactory.getLogger(AgvMsgWorkerRunnable.class);
    ...
    private void _processAgvReport(String[] tokens) {
        Vhl vehicle;
