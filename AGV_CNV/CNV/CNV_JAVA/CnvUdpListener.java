...
/**
 * CNV (Conveyor) UDP Listener for STK MCP7/SSS message reception.
 * Handles incoming UDP packets for CNV Text ID:4 (equipment/machine state reports).
 * Adapted from OhtUdpListener.java for Conveyor-specific message processing.
 *
 * Supports single and multi-listener modes for port sharing across multiple FABs.
 * Features UDP monitoring with 30-second logging intervals.
 */
public class CnvUdpListener {
    ...
    public CnvUdpListener(String fabId, String mcpName, int port) {
        this.fabId         = fabId;
    ...
    public CnvUdpListener(String fabId, String mcpName, String ipString, int port) {
        this.port = port;
    ...
    public void start() {
        ...
        if (!isMultiListener) {
            ...
            receiveThread = new Thread("CnvMessageQueuing") {
                public void run(){
                    ...
                    try {
                        ...
                        while (isRunning) {
                            try {
                                ...
                                logger.error("An Error while processing [fab: {} | mcp: {}] CNV Message [packet: {}]", fabId, mcpName, packet, e);
                            }
                        }
                    } catch (SocketException e1) {
                        logger.error("An Error while opening [fab: {} | mcp: {}] CNV DatagramSocket [{}] ", fabId, mcpName, port, e1);
                    }
                }
            };
        } else {
            receiveThread = new Thread("MultiCnvMessageQueuing") {
                public void run(){
                    DatagramPacket packet     = null;
                    byte[] buffer;
                    ...
                    try {
                        ...
                        logger.error("An Error while opening [port: {}] CNV DatagramSocket [{}] ", port, e1);
                    }
                }
            };
        }
