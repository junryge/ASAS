...
class HeaderWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Ralph 반복 횟수
        ralph_label = QLabel("반복")
        ralph_label.setStyleSheet(f"""
            color: {COLORS['text_muted']};
            font-size: 10px;
            font-weight: 700;
            letter-spacing: 1px;
        """)
        layout.addWidget(ralph_label)

        self.ralph_spin = QSpinBox()
        self.ralph_spin.setRange(1, 20)
        self.ralph_spin.setValue(5)
        self.ralph_spin.setSuffix("회")
        self.ralph_spin.setToolTip("Ralph 자율 루프 최대 반복 횟수")
        self.ralph_spin.setFixedWidth(70)
        self.ralph_spin.setFixedHeight(28)
        self.ralph_spin.setStyleSheet(f"""
            QSpinBox {{
                font-size: 11px;
                padding: 2px 4px;
                border-radius: 6px;
        ...
        """)
        layout.addWidget(self.ralph_spin)
