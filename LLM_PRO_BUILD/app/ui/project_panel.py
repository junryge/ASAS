...
class ProjectPanel(QWidget):
    ...
    def _delete_project(self):
        ...
        if not self.current_project_id:
            QMessageBox.warning(self, "알림", "삭제할 프로젝트가 없습니다.")
            return
        ...
        reply = QMessageBox.question(
            ...
            f"프로젝트 폴더와 모든 파일이 삭제됩니다.\n이 작업은 되돌릴 수 없습니다.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            from app.aider.project_manager import delete_project
            success = delete_project(self.current_project_id)
            if success:
                self.current_project_id = None
                self.file_tree.clear()
                self.file_viewer.set_code("# 프로젝트가 삭제되었습니다")
                # 프로젝트 목록 새로고침 요청
                self.project_changed.emit("")
            else:
                QMessageBox.warning(self, "오류", "프로젝트 삭제에 실패했습니다.")
