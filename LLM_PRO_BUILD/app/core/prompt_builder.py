...
정확하고 실용적인 정보, 예제 코드 포함. 한국어로 친절하게.""",
    ...
    "code_assistant": """당신은 시니어 코드 분석 전문가입니다.
코드를 깊이 분석하여 실용적이고 구체적인 피드백을 제공합니다.
규칙:
- 한국어로 설명
- 마크다운 형식 사용
- 코드 예제 포함
...
ACTION_PROMPTS = {
    ...
    # 코드 어시스턴트 퀵 액션
    "ca_explain": "이 코드를 상세히 설명해주세요. 전체 목적, 각 함수/클래스의 역할, 실행 흐름, 사용된 패턴을 분석하세요.",
    "ca_find_bugs": "이 코드에서 버그, 잠재적 문제, 엣지 케이스, 보안 취약점을 찾아주세요. 각 이슈의 심각도(높음/중간/낮음)와 수정 방법을 제시하세요.",
    "ca_suggest_improvements": "이 코드의 개선점을 제안해주세요: 가독성, 성능, 베스트 프랙티스, 디자인 패턴. 구체적인 개선 코드를 포함하세요.",
    "ca_generate_tests": "이 코드에 대한 포괄적인 단위 테스트를 작성해주세요. 정상 케이스, 엣지 케이스, 에러 케이스를 모두 커버하세요.",
    "ca_add_docstrings": "이 코드의 모든 함수와 클래스에 상세한 docstring/주석을 추가해주세요. 파라미터, 반환값, 예외, 사용 예시를 포함하세요.",
    "ca_refactor": "이 코드를 리팩토링해주세요. SOLID 원칙, 가독성, 유지보수성을 기준으로 개선된 전체 코드를 제공하세요.",
}
