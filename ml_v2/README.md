...
표준 라이브러리만으로 돌아가고, Chronos-2 있으면 자동으로 실모델을 쓴다.
...
| `main.py` | 학습 → 평가 (`learn` / `sweep` / `all`) |
| `realtime.py` | **실시간 러너** — 매분 수집 CSV 감시 → 판정·경보 |
| `aws_idc_history_downloader.py` | (보조) 과거 기간 DB→날짜별 CSV 다운로드 |

### 📖 문서 어디부터 읽나
| 목적 | 파일 |
|---|---|
| **이어서 개발·수정** | **`HANDOVER.md`** ← 전체 문서 (설계근거·알고리즘·스키마·지뢰) |
| 그냥 돌리기 | `실행명령어.md` (복붙용) |
| 요약 인수인계 | `인수인계.md` |
| 성적 상세 | `RESULTS.md` |
| 고객 설명 | `docs/고객설명_조기경보.html` |
