...
public class CnvMsgWorkerRunnable implements Runnable {
     ...
     * @param msg
     */
	public CnvMsgWorkerRunnable(String fabId, String cnvId, String msg, long receivedMilli) {
		Eqp eqp = DataService.getDataSet().getAllEqpNameMap().get(cnvId);
		this.msg = msg;
		...
		try {
			//logger.info("CnvMsgWorkerRunnable >>>>> cnvId:{}, msg:{}, msgSeq:{}, fabId:{}, cnv:{}", cnvId, msg, msgSeq, fabId, cnv==null ? "null":"ok");
			logger.info("CnvMsgWorkerRunnable >>>>> cnvId:{}, msgSeq:{}, fabId:{}, cnv:{}", cnvId, msgSeq, fabId, cnv==null ? "null":"ok");
	...
	public void processMsg() {
	    ...
	    try {
	    	//logger.info("{} seq[{}] waited for {}ms msg : [{}] {} ", this.cnvId, msgSeq, System.currentTimeMillis() - receivedMilli, msgType, msg);
	    	logger.info("{} seq[{}] waited for {}ms msg : [{}] ", this.cnvId, msgSeq, System.currentTimeMillis() - receivedMilli, msgType);
		...
		try {
			...
			if(task != null) {
				...
				if(StringUtils.isNotEmpty(zoneIdCurrent)) {
					...
					logger.warn("{} seq[{}] task {} zoneIdCurrent is {} msg : [{}] {} ", this.cnvId, msgSeq, taskId, zoneIdCurrent, msgType, dataStr);
				}
			}else {
				logger.warn("{} seq[{}] task {} is null msg : [{}] {} ", this.cnvId, msgSeq, taskId, msgType, dataStr);
			}
			saveCnvTaskToLogpresso(task);
		}catch(Exception e) {
