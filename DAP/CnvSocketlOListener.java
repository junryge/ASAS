...
public class CnvSocketIOListener {
	...
	public CnvSocketIOListener(String fabId, String cnvId, String urlString) {
		...
		try {
			...
			this.ip 		= ip;
			this.port 		= port;			
			this.daemon 	= DataService.getInstance().getFabPropertiesMap().get(fabId).getConveyorDaemon();
			this.subject 	= DataService.getInstance().getFabPropertiesMap().get(fabId).getConveyorSubject();
	...
	public boolean connectAndBuildCnvRawLayout() {
		...
		try {
			...
			logger.info("started connect conveyor socket :: " + connStr);
			...
			socket.on("message", new Emitter.Listener() {
			    ...
			    public void call(Object... args) {	//	Object... = Object[]
			    	logger.info("confirm socket arguments" + args);
			    	...
			    	for(Object arg : args) {
				    	...
				    	try {
							...
							if (
								...
								logger.info("{} initializedata received : {}", cnvId, received.getString("data"));
								...
								logger.info("{} UpdateZoneState received : {}", cnvId, dataJo.toString());
			...
			this.socket.on(Socket.EVENT_CONNECT_ERROR, new Emitter.Listener() {
			    ...
			    public void call(Object... args) {
			    	...
			    	logger.warn("connect error : {}", socket.id());
			    }
			});
			...
			logger.info("finished...conveyor initializion");
		} catch (Exception e) {
...
//				            TibrvSendToLocalDaemon.sendMessage(msg);
				    		...
				    		TibrvAPI.send("", "", daemon, subject, message);
				    		
				    		// worker 미사용으로 인한 로직 제거
				    		Msg data = new Msg(
				    				fabId, 
				    				MSG_TYP.CNV, 
    ...
    public void listenStop() {
        if (this.socket != null) {
            if (this.socket.connected()) {
            	this.socket.disconnect(); // 연결 끊기
            }
            this.socket.close(); // 자원 해제
            this.socket = null; // 참조 제거
            logger.info("Socket closed successfully.");
        } else {
        	logger.warn("Socket is already closed or not initialized.");
        }
    }
