...
| 모드 | 동작 | 안전도 |
|---|---|---|
| **`api`** (기본) | 윈도우에 "화면 켜둬"라고 요청(SetThreadExecutionState). **마우스·키 입력을 전혀 보내지 않음** | ★★★ |
| `key` | F15 키를 누름. 실제 키보드에 없는 키라 어떤 프로그램도 반응하지 않음 | ★★☆ |
| `mouse` | 마우스를 3px 움직였다 되돌림 (클릭 없음) | ★☆☆ |
| `click` | 마우스 이동 + 클릭 | ✗ 엉뚱한 곳이 눌릴 수 있어 권장하지 않음 |

```json
{ "keep_awake": true, "keep_awake_mode": "api", "keep_awake_interval": 60 }
```

- 어떤 모드든 **RPA 실행 중에는 개입하지 않는다.**
- `api` 는 화면 꺼짐·절전·화면보호기를 막는다. 회사 보안정책의 **자동 잠금**까지 막아야 하면
  `key` 로 바꾸거나(입력이 있어야 유휴시간이 초기화되는 정책), 두 대의 방식 중 맞는 쪽을 쓴다.
