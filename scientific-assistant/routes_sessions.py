...
엔드포인트:
    POST   /api/code/session/save              (body: {session_id?, messages, skills_used?, metadata?, uploaded_files?})
    GET    /api/code/session/load/<sid>
    GET    /api/code/session/list
    DELETE /api/code/session/delete/<sid>
"""
...
def _route_exists(app_or_bp, rule: str, method: str) -> bool:
    # Blueprint는 url_map 속성이 없음(등록 시점에 부모 app으로 흡수됨)
    # → 가드 스킵하고 그냥 등록 진행. 중복 시 Flask가 자체 에러로 알려줌.
...
def register_session_routes(app) -> int:
    ...
    if not _route_exists(app, "/api/code/session/save", "POST"):
        @app.route("/api/code/session/save", methods=["POST"], endpoint="ca_session_save")
    ...
    if not _route_exists(app, "/api/code/session/load/<session_id>", "GET"):
        @app.route("/api/code/session/load/<session_id>", endpoint="ca_session_load")
    ...
    if not _route_exists(app, "/api/code/session/list", "GET"):
        @app.route("/api/code/session/list", endpoint="ca_session_list")
        def _ca_session_list():
            return jsonify({"sessions": _list()})
        registered += 1

    if not _route_exists(app, "/api/code/session/delete/<session_id>", "DELETE"):
        @app.route("/api/code/session/delete/<session_id>", methods=["DELETE"], endpoint="ca_session_delete")
