...
대시보드를 열면 먼저 **시스템 선택 화면**이 뜬다.

| | 시스템 | 지금 상태 |
|---|---|---|
| **ALL** | 전체 시스템 통합 | **운영 중** — 지금 돌고 있는 관제가 이것이다 |
| M14 / M14B / M16A / M16B / M14HUB | 각 FAB | 준비 중 — 화면 자리만 잡아 뒀다 |

* **고르기 전에는 수집이 시작되지 않는다.** 안 고른 화면이 데이터로 차 버리면
  지금 뭘 보고 있는 건지 알 수 없다.
* 고르면 주소가 `?sys=ALL` 로 바뀐다 → **새로고침해도 그대로 그 시스템으로** 들어간다.
  관제 화면은 하루 종일 떠 있으니 새로고침할 때마다 다시 고르게 하면 안 된다.
* 상단 바 왼쪽의 `ALL 바꾸기` 칩을 누르면 다시 고를 수 있다 (`Esc` 로 닫기).
* 준비 중인 FAB 은 눌러도 들어가지 않는다.

**FAB 을 하나 붙일 때** — `static/dashboard.html` 의 `SYSTEMS` 목록에서 그 줄의
`ready` 를 `true` 로 바꾸면 된다. 목록이 유일한 정의라 화면 다른 곳에 FAB 이름을
또 적을 필요가 없다. (수집이 실제로 붙기 전에 켜면 빈 화면만 보게 된다 —
`tests/test_dashboard_open.py` 가 이걸 막는다.)
