...
* 저장된 날짜는 드롭다운에 행수까지 나온다 (`data/` 의 `YYYYMMDD_TOTAL.CSV`)
* 목록에 없는 날짜는 날짜 입력칸에 직접 넣고 `조회`
* CSV 가 없는 날짜면 `확보` 로 그 날짜를 로그프레소에서 새로 가져온다
* `CSV` 는 그 날 원본 파일 내려받기

## 스코어 추이 스트립 (항상 보이는 그래프)

실시간 관제·과거 데이터 조회 두 탭 모두 표 위에 **하루치 스코어 추이**가 항상 떠 있다.
더블클릭할 필요 없이 계속 보인다.

* X축 00:00~24:00 고정 — 아침부터 지금까지가 한눈에
* 등급 밴드(50/71/85) 배경, 경계 이상 지점은 등급 색 점, 최고점 `▲NN점`
* 오늘이면 현재 시각 세로선
