...
```bash
python jupyter_csv.py --list                         # ★먼저 어느 날짜가 있는지 본다
...
`--list` 를 먼저 보라. 예측 잡이 오래된 파일을 지우는 경우가 흔해서, 없는
날짜를 잔뜩 요청하면 404 만 잔뜩 받는다. 목록은 주피터의 `/api/contents`
API 로 읽는다 (막아 둔 서버면 그렇다고 알려주고, 날짜를 직접 지정하면 된다).

```
서버에 있는 날짜 파일 3개
  20260809  20260809_발동이벤트.csv  412KB  2026-08-09T23:59:00
  20260810  20260810_발동이벤트.csv  418KB  2026-08-10T23:59:00
  20260811  20260811_발동이벤트.csv  405KB  2026-08-11T23:59:00
```
...
50점대 알람이 너무 잦아 경계 하한을 50→60 으로 올렸다 (2026-08). 60점 미만은 알람 없음(정상 운영).
...
## 오프닝 — 관제할 시스템 고르기 (전부 실시간)

대시보드를 열면 먼저 **시스템 선택 화면**이 뜬다. 여섯 화면 모두 같은 주기로
실시간 수집된다.

| 시스템 | 데이터 파일 | 점수 |
|---|---|---|
| **ALL** (전체 통합) | `predict_tobe/{day}_발동이벤트.csv` | `unified_risk_score` |
| M14 / M14B / M16A / M16B / M16HUB | `predict_tobe/fab분리/{day}_발동이벤트_{FAB}.csv` | **`area_score`** (그 구역 자기 점수) |

**FAB 파일의 함정 — 받는 순간 정규화한다.** FAB 파일에도 전체 점수
(`unified_risk_score`)와 전체 기준 `hot_area`(보통 M16HUB)가 그대로 들어 있다.
그래서 받는 즉시 `area_score`→점수, FAB 코드→`hot_area` 로 바꿔 저장한다
(`jupyter_csv._fab_rows`). 안 바꾸면 M14 화면이 전체 점수로 등급을 매기고
케이스가 M16HUB 로 찍힌다. 원본 값은 `all_score`/`all_hot_area`/`all_level`
컬럼으로 남는다.

* 저장은 **`data/{FAB}/` 아래로 완전히 분리** — 날짜 CSV·케이스·LLM 판단·
  리포트·분석 전부. ALL 은 기존처럼 `data/` 바로 아래.
* 고르면 주소가 `?sys=M14` 로 남는다 → 새로고침해도 그 시스템으로 들어간다.
  상단 칩으로 전환하면 그 시스템으로 새로 연다 (섞임 방지).
* 고르기 전에는 화면 폴링이 시작되지 않는다. (서버 쪽 수집은 여섯 시스템
  모두 항상 돈다 — 안 보고 있어도 데이터는 쌓인다.)
* **분당 LLM 판단은 여섯 시스템 전부** 돈다 — 'LLM 판단 일치' 가 모든 화면에서
  채워진다. 게이트웨이는 한 번에 하나만 친다 (전 시스템이 `accuracy._busy`
  락으로 직렬화 + 시스템별 시작 시차 4초, 밀린 분은 다음 주기에 과거로
  거슬러 메움). 부담스러우면 `config.llm.fab_minute` 를 `"watched"`(보고
  있는 FAB만) 또는 `"off"` 로. ALL 은 설정과 무관하게 항상 돈다.
* 파일명이 화면 코드와 다르면 `config.source.jupyter.fabs` 의 값(접미사)만
  바꾸면 된다. 화면 목록(`SYSTEMS`)과 config 가 어긋나면
  `tests/test_fabs.py` 가 실패한다.

**FAB 별 과거 데이터 채우기**

```bash
python jupyter_csv.py --backfill all --sys M14      # M14 파일이 있는 날짜 전부
python jupyter_csv.py --backfill 30 --sys M16HUB    # 최근 30일
for f in M14 M14B M16A M16B M16HUB; do              # 다섯 FAB 한꺼번에
  python jupyter_csv.py --backfill all --sys $f
done
```
...
* `daily.py` 가 저장된 날짜 CSV 에서 **③ 사건목록 · ④ AMOS 표**를 만든다 —
  `m16_hub_skills/발동이벤트_요약.py` 와 **같은 규칙**(점수 60+(임계) · 간격 60분 ·
...
* 정상(60점 미만) 구간은 **짧게** 묻는다 (예/아니오 + 한 줄). 하루 1440번이라 낭비를 줄인다
...
|---|---|---|
| 예 | 임계(60점) 이상 **5분 연속 유지** or 등급 상승 | **적중** |
...
|---|---|---|---|---|---|---|---|
| 08:05 | 41점 | 예 | 70% | 상승 추세 지속 | 적중 | 이후 9분 연속 60점 이상 유지 · 최고 88점 | 정탐 오탐 |
| 08:14 | 61점 | 예 | 80% | 리프터 정체 | 과다탐지 | 5분 내 60점 아래로 회복 · 최고 44점 | 정탐 오탐 |
| 08:16 | 38점 | 아니오 | 60% | 회복 국면 | 적중 | 창 안 최고 31점 — 계속 60점 아래 | 정탐 오탐 |
...
```
┌ 스코어 패널 ── unified_risk_score, 등급 밴드(60/71/85), 사건·최고점 표시
