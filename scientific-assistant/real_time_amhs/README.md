...
값은 CSV 의 **실제 컬럼 값 그대로**이고, 화면에도 raw 컬럼명이 같이 뜬다
(`M16HUB_ra`, `M16HUB_stb_util` …).
...
`key` 가 **CSV 실제 컬럼명**이고, 그 날 CSV 에 값이 없는 컬럼은 목록에서 자동으로 빠진다.

```json
"ui": { "strip_metrics": [
  {"key":"unified_risk_score","label":"스코어","unit":"점","color":"#3DDBE8","max":100,"bands":true},
  {"key":"M16HUB_ra","label":"M16HUB 반송시간","unit":"분","color":"#FF6B5E"},
  {"key":"M16HUB_stb_util","label":"M16HUB STB저장율","unit":"%","color":"#F2C94C","max":100}
]}
```
