...
def numbers_of(text):
    """밖에서 근거 숫자를 보태 넣을 때 (MCP·첨부 등).

    ★근거는 관제만이 아니다. MCP 로 받아 온 글(위키·요청이력)의 숫자도
      근거다 — 안 보태면 가드가 그걸 '지어낸 수' 로 보고 **답을 통째로
      버린다.** 실제로 위키의 호기명(4AFC3201)·층수(3F)가 그렇게 걸려서,
      연결 경로를 물었는데 엉뚱한 관제 상태 요약이 나갔다.
    """
