...
# ★exploratory-data-analysis 는 과학 파일 포맷(.pdb/.fasta/.bam) 카탈로그라
#   관제 데이터 분석에 쓸모가 없다 — 등록하면 "분석" 질문마다 걸려서
#   분자동역학 얘기를 물고 온다. 뺐다. 분석 방법은 data-analysis 스킬이 맡는다.
SEED_DOCS = {
...
SEED_LOCAL = [
    ...
    ("m16_hub_skills/페르소나_통합.txt", "관제_보고_페르소나.txt"),
    # ★발동이벤트_요약.py 는 등록하지 않는다 — 스크립트라 자료로서 값이 작고,
    #   비밀 검사가 정규식 변수(_RULE_TOKEN_RE)를 토큰으로 오탐한다. 계산 방식은
    #   data-analysis 스킬에 글로 적어 뒀다 (거기가 읽기도 낫다).
    # 우리가 쓴 관제 데이터 분석 방법 — 스킬로도 심지만 자료 탭에서도 보이게
    ("analysis_skills/data-analysis/SKILL.md", "데이터분석_관제데이터_보는법.md"),
]
