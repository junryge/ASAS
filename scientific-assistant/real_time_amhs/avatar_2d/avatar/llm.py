...
# ── '뜻을 묻는 꼴' — 값이 아니라 **무엇인지**를 묻는다 ────────────────
# "HUBROOM 지표가 무엇이 있어?" 는 지식 질문인데 '지표' 가 DATA_WORDS 에
# 걸려서 관제 근거가 붙었고, 서윤이 그날 14:48 수치를 읊었다 —
# **물어본 적 없는 것**이다 ("내가 지금 데이터 안 물어봤는데?").
# 뜻·종류·역할을 묻는 꼴이면 관제 근거를 안 붙인다. 그러면 위키와 페르소나로
# 답한다.
DEF_WORDS = ("뭐야", "뭐지", "뭔지", "뭔가", "무엇", "무슨 뜻", "무슨뜻",
             "의미", "뜻이", "설명해", "설명 해", "종류", "역할", "차이",
             "어떤 게", "어떤게", "어떤 것", "뭐가 있", "뭐 있", "정의",
             "가르쳐", "알려줘?")

# 값·상태를 묻는 말. 하나라도 있으면 뜻 질문이 아니다.
STATE_WORDS = ("지금", "현재", "오늘", "어제", "그제", "방금", "실시간",
               "몇 점", "몇점", "얼마", "어때", "떴", "발생", "울려",
               "괜찮", "이상 있", "위험해", "심각", "올랐", "내렸", "추이")

# ★이 말이 있으면 뜻 질문으로 보지 않는다. "M16HUB 점수 뭐야?" 는 '뭐야' 가
#   있어도 지금 점수를 묻는 것이다 — 여기서 근거를 빼면 답을 못 한다.
_HARD_DATA = ("점수", "스코어", "등급", "알람", "경계", "초위험",
              "m14", "m16", "sla", "maxcapa")


def is_def_question(text):
    """뜻·종류를 묻는 꼴인가 (관제 수치가 필요 없는 질문)."""
    t = str(text or "").lower()
    if any(w in t for w in _HARD_DATA) or any(w in t for w in STATE_WORDS):
        return False
    return any(w in t for w in DEF_WORDS)


def is_data_question(text):
    t = str(text or "").lower()
    if not any(w in t for w in DATA_WORDS):
        return False
    # ★뜻을 묻는 꼴이면 관제 수치를 붙이지 않는다 — 안 물어본 것이다.
    return not is_def_question(t)
...
AGENT_RULES = (
    ...
    "1-0. ★답할 때 **순서**가 있다. 이 순서를 지켜라.\n"
    "  ① 먼저 [외부 도구 — MCP] · [참고 자료] · [관제 근거] 블록을 읽는다. "
    "거기 있으면 **그것이 답이다.** 네가 처음 보는 이름이라도, 블록에 "
    "적혀 있으면 아는 것이다 — 블록을 안 읽고 '모른다' 고 하면 안 된다. "
    "위키에 올려 둔 글을 못 봤다고 하는 것이 제일 나쁜 답이다.\n"
    "  ② 블록에 없고 **관제 이야기**면 '지금은 확인이 안 돼요' 라고 한다.\n"
    "  ③ 블록에 없고 관제와 상관없는 일이면, **널리 알려진 것에 한해** "
    "네가 아는 대로 답한다 (공개된 제품·회사·일반 상식). 이때 "
    "'등록된 자료에는 없는데' 라고 한 번 밝힌다. 관제 수치·등급·구역 "
    "이름은 여기서 **절대 지어내지 않는다** — 그건 ② 로 간다.\n"
    "  ④ ★그러나 **모르는 이름은 절대 지어내지 마라.** 사람 이름, 캐릭터 "
    "이름, 사내 용어, 처음 듣는 제품명처럼 **네가 확인할 수 없는 이름**을 "
    "물으면, 블록에 없는 한 답은 하나뿐이다: "
    "\"등록된 자료에 없고 저도 모르는 이름이에요. 위키에 올려 주시면 "
    "그때부터 answer 할 수 있어요.\" "
    "이때 **추측으로 채우지 마라** — '가상의 인물이에요' · '~인 것 "
    "같아요' · '이야기 속에서 중요한 역할을 해요' 처럼 그럴듯하게 "
    "지어내는 것이 제일 나쁜 답이다. 모른다고 하는 편이 낫다. "
    "이름을 두고 아무 설명이나 붙이느니 아무 말도 안 하는 게 낫다.\n"
    "  ⑤ 반쯤 아는 것은 어디까지 아는지 **선을 그어** 말한다 — "
    "'이름은 들어 봤는데 자세한 건 모릅니다' 처럼.\n"
    "1-0-1. **물어볼 수 있다.** 다만 되묻기 전에 1-0 의 ① 을 먼저 한다 — "
    "블록에 답이 있는데 되물으면 사람이 화를 낸다. "
    "블록을 다 읽고도 질문이 모호하거나(무엇을 말하는지 여럿), 빠진 조건이 "
    "있거나(어느 FAB·언제·어느 파일), 답이 갈리는 자리면 지어내지 말고 "
    "**되물어라.** 한 번에 하나만, 짧게. "
    "예: \"M14 말씀이신가요, 아니면 M14B 인가요?\" · "
    "\"어느 날짜 기준으로 볼까요?\" "
    "되물을 때는 지금까지 알아낸 것을 한 줄로 먼저 말하고 묻는다 — "
    "아무것도 없이 묻기만 하면 사람이 두 번 일한다.\n"
...
def build_messages(persona, user_text, history, doc_store, settings,
    ...
    if mcp_text:
        ...
        # ★분량 규칙을 여기서 풀어 준다. 페르소나에 "1~3문장, 목록 금지" 가
        #   있는데 그건 **잡담용**이다. 지식 설명에까지 걸면 "LFT 는
        #   리프터예요" 한 줄로 끝나고, 층간 반송이라는 것도 ZT 라고도
        #   부른다는 것도 다 잘린다 — 위키를 읽어 온 보람이 없다.
        wiki_rules = ("· [AMHS 위키] 는 **지식 문서**다. 지금 수치가 아니다 —"
                      " 위키에 적힌 숫자를 현재 값으로 말하면 안 된다.\n"
                      "· 용어·구조·경로를 물으면 이 내용으로 답한다. 위키에"
                      " 없는 것은 없다고 말한다 — 지어내지 않는다.\n"
                      "· ★**여기서는 짧게 줄이지 마라.** 페르소나의 분량 제한"
                      "(1~3문장·목록 금지)은 잡담에 거는 것이고, 지식 설명에는"
                      " 걸지 않는다. 물어본 것에 대해 위키에 있는 만큼 **끝까지"
                      " 풀어서** 답한다. 말투는 **페르소나 그대로**다.\n"
                      "· 여러 가지를 말할 때는 **줄바꿈으로 나눠** 쓴다"
                      "(장치별·구간별·단계별). 한 문단에 붙여 쓰면 관제"
                      " 화면에서 못 읽는다.\n"
                      "· **호기명·장치명·컬럼명은 원문 그대로** 옮긴다."
                      " `6ABL60~` 를 '6ABL 계열' 로 뭉개지 마라 — 현장이 아는"
                      " 이름이 그것이다. 다른 이름이 있으면 같이 말한다"
                      "(리프터는 ZT 라고도 부른다는 식으로).\n"
                      "· 경로를 물으면 **거쳐 가는 순서를 다 적는다.**"
                      " 중간을 건너뛰면 그 경로는 쓸 수가 없다.\n"
                      "· 본문이 '뒤가 잘렸다' 로 끝나면 그 뒤는 못 본 것이다."
                      " 안 본 부분을 아는 것처럼 말하지 마라.\n") if wiki_in else ""
        sysmsg += ("[외부 도구 — MCP]\n" + terms.no_code(mcp_text) +
                   "\n· ★여기 적힌 것은 **우리가 등록해 둔 자료**다. 네가 "
                   "처음 보는 이름이라도 여기 있으면 그것으로 답해라 — "
                   "'모른다' 고 하면 안 된다.\n"
    ...
    if ctx:
        _seg(parts, "docs", ctx)
        sysmsg += ("[참고 자료]\n" + terms.no_code(ctx) +
                   "\n★위 자료에 있으면 **그것이 답이다** — 처음 보는 "
                   "이름이라도 여기 적혀 있으면 아는 것이다. "
                   "자료에 없는 **관제 수치**는 아는 척하지 않는다. "
                   "관제와 상관없는 일반 지식은 자료에 없어도 아는 대로 "
                   "답하되, 모르면 모른다고 말하고 필요하면 되묻는다.\n"
