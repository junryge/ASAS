...
# 질문 상투어 — 검색어에서 뺀다. 문서에 없는 말이라 점수만 흩뜨린다.
#   ★조사·어미가 붙어 오므로 '시작하는 말' 로 본다 (알고/알고있는/알려줘…).
_FILLER = (
    "너가", "니가", "네가", "당신", "너는", "저는", "제가", "우리",
    "알고", "알아", "알려", "아는", "모르", "몰라", "몰랐", "확인", "설명",
    "말해", "말좀", "가르", "궁금", "질문",
    "내용", "관련", "대해", "대한", "그거", "이거", "저거", "그건", "이건",
    "무엇", "뭐야", "뭔데", "뭐지", "뭐냐", "어떻", "어때", "인가", "인지",
    "좀", "그", "저", "이", "요", "해봐", "해줘", "주라", "주세요", "봐줘",
    "있는", "있나", "있어", "하는", "한번", "한", "및", "또", "그리고",
)
...
def _worth(w):
    ...
    ★영문·숫자·코드(LFT · 4AFC3201 · M16WT)는 **무조건 남긴다** — 짧아도
      그게 제일 센 신호다. 한글만 상투어를 걸러낸다.
    """
    w = re.sub(r"[^0-9A-Za-z가-힣_.~]", "", str(w or ""))
    if not w:
        return False
    if re.search(r"[0-9A-Za-z]", w):
        return True                       # 영문·숫자가 섞였으면 남긴다
    if len(w) < 2:
        return False
    return not any(w.startswith(f) for f in _FILLER)
...
def _arg_of(text, spec):
    ...
    if kind == "oneof":
        ...
        return None
    if kind == "text":
        # ★질문에서 **찾을 말만** 남긴다. 그냥 질문을 통째로 넘기면 BM25 가
        #   군말에 점수를 나눠 주고 아무것도 못 찾는다 — 실제 증상:
        #     "AMHS 위키 너가 알고 있는 내용 확인해봐"
        #   에서 '너가·알고·있는·내용·확인해봐' 가 문서에 없어서 검색이
        #   빈손으로 왔고, 서윤은 "위키에 그런 내용이 없어요" 라고 답했다.
        #   위키에는 멀쩡히 있는데.
        t = re.sub(r"\s+", " ", str(text or "")).strip()
        keep = [w for w in t.split(" ") if _worth(w)]
