# -*- coding: utf-8 -*-
"""real_time_amhs/amos_block.py — 사건발생 보고서 HTML에 AMOS 인터랙티브 블록 주입.

★데모스 demos_v1/amos_report.py 와 같은 내용의 **독립 복사본**이다.
  데모스를 import 하지 않는다 (이 폴더는 별도 프로세스로 돈다).
  체크박스·시간 분리·O/X 판정 규격이 데모스 개인 에이전트 보고서와 동일해야 하므로
  demos_v1/amos_report.py 를 고치면 이 파일도 같이 맞춘다.
...
def amosify(body_html):
    ...
    ★2번 AMOS 표가 비어도(사건 0건이거나 '내역 없음' 한 줄이어도)
