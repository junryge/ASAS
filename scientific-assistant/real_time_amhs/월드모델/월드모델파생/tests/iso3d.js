...
/* ★2026-09 — 2D 에 '삼각형 안의 점' 이 생겼지만 아이소메트리는 회의 뒤로 미뤘다.
   차량 색은 예전 그대로라 3D 는 건드릴 것이 없다. 점도 2D 에만 찍는다. */
ok(!/drawCarryDot/.test(H.slice(H.indexOf('===== 3D 아이소메트리 (시작) ====='),
                                H.indexOf('===== 3D 아이소메트리 (끝) ====='))),
   '점은 2D 에만 — 3D 절에 drawCarryDot 이 들어가면 안 된다');
