...
| `walls: false` | 벽을 안 세운다. 돌려 보면 안쪽을 가리기만 하고 정보가 없다 (고객 지시). 바닥도 두께 없는 평면으로 깐다 — 0.8 m 상자면 그 옆면이 낮은 벽처럼 둘러싼다. |
| `railScale` 기본 **0.45** | ★원본 레일 판은 0.56 m 인데 실물 M14A 는 평행 레일 사이가 **중앙값 0.30 m**(p25 0.22 m) 라, 재 보니 **83%** 가 겹쳐 한 덩어리로 보였다(고객: "레일이 하나로 뭉쳐서 있어"). 0.45 면 판이 0.25 m 라 두 줄로 갈린다. |
| `portScale` · `textScale` + 바의 **'크기'** 패널 | 레일 굵기·차량·설비·글자 배수를 3D 안에서 슬라이더로. 2D 설정(⚙)과 섞지 않는다 — 2D 차량 크기는 px 배수고 3D 는 m 배수라 뜻이 다르다. `sizechange` 이벤트로 화면이 localStorage 에 따로 저장한다. |
