...
class 조회한_것을_대화_안에서_기억한다(_Fake):
    ...
    def test_이어받은_것은_이어받았다고_밝힌다(self):
        ...
        # ★어디서 온 글인지 이름을 적는다. 무엇을 들고 왔든 "요청이력이다"
        #   라고 하면, 위키를 들고 왔을 때 서윤이 엉뚱한 것으로 알고 답한다.
        self.assertNotIn("요청이력이다", nxt)
...
class 위키_설정이_말이_된다(unittest.TestCase):
    ...
    def test_AMOS_질문에_걸린다(self):
        """★AMOS(지능형 통합 AMHS 모니터링 시스템) 화면 지식도 위키에 있다.
        어느 메뉴에서 무엇을 보는지를 물으면 위키를 봐야 한다."""
        for q in ("AMOS 가 뭐야?", "AMOS 메뉴 뭐가 있어",
                  "Site 모니터링은 뭘 보는 거야", "Layout 모니터링에서 뭐 확인해?",
                  "Custom 모니터링 어떻게 쓰는 거야", "FAB 상세 모니터링은 뭐가 달라",
                  "통신 이상은 어떻게 감지해?", "MES 랑 MCS 통신 끊기면 어디서 봐",
                  "서버 리소스 이상 감지가 뭐야", "CPU 임계 넘으면 어떻게 되나",
                  "Alarm 설정 어디서 해?", "알람 이력 어디서 봐",
                  "연락처 관리는 뭐하는 메뉴야",
                  "AI Agent Chatbot 으로 뭘 물어볼 수 있어", "챗봇 질의 예시 알려줘"):
            self.assertIn("wiki", self.keys(q), q)
    ...
    def test_관제_질문에는_안_걸린다(self):
        ...
        ★AMOS 를 넣으면서 특히 조심한 것 — 낱말 `알람`·`모니터링` 은
          **안 넣는다.** 넣으면 "지금 알람 떴어?" 같은 관제 질문마다 위키를
          뒤진다. `MCP` 도 안 넣는다 — 아바타 자신의 MCP 설정 질문이 걸린다.
        """
        for q in ("M16HUB 지금 몇 점이야?", "M14 반송시간 알려줘",
                  "ALL 점수 얼마야", "지금 상태 어때", "M16B 어때",
                  "M14 반송 어떻게 되고 있어?", "어제 8시에 어땠어?",
                  # AMOS 낱말을 넣으면서 새로 지켜야 할 것들
                  "지금 알람 떴어?", "알람 해제해줘", "FAB 모니터링 중이야?",
                  "모니터링 어떻게 되고 있어", "MCP 연결해줘", "MCP 끊어줘",
                  "지금 경계 몇 개야"):
            self.assertNotIn("wiki", self.keys(q), q)

    def test_흔한_낱말을_안_넣는다(self):
        """★위 시험이 통과해도, 누가 편하자고 '알람' 을 넣으면 그날 깨진다.
        낱말 자체를 못 넣게 막아 둔다."""
        w = [s for s in config.MCP_SERVERS if s["key"] == "wiki"][0]
        for bad in ("알람", "Alarm", "모니터링", "MCP", "AI", "이상", "서버"):
            self.assertNotIn(bad, w["when"],
                             "'%s' 는 관제 질문마다 위키를 뒤지게 만든다" % bad)
