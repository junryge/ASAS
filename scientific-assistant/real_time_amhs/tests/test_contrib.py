...
★점수식을 푼 게 아니라 평소 대비 편차로 낸 **추정**이다. 그래서 눈금이
  깨지는 경우가 실제로 있었다 — 리프터막힘처럼 평소가 대부분 0 인 지표는
...
def make_rows(rev_quiet=0.0, stb_flat=False):
    """정상 284분 + 사건 16분. rev_quiet=0 이면 리프터막힘이 평소 전부 0."""
    rnd = random.Random(3)
    rows = []
    for i in range(300):
        ev = i in EV
...
class Contrib(unittest.TestCase):
    ...
    def test_MAD_0_인_지표가_100퍼센트를_먹지_않는다(self):
        """★실제로 z=6000000000 이 나와 리프터막힘이 100%를 차지했다."""
        d = self.peak()
        top = d["items"][0]
    ...
    def test_발동한_지표가_상위에_온다(self):
        ...
        self.assertTrue(any("리프터막힘" in t for t in top3), top3)
