...
# ★임계값은 **컬럼흐름 상세도 2026-09-21 판**(FAB별 HTML, 5절 계산식)에 맞췄다.
#   그 전 값(M16B R-A 3.5 · R-D 95.0 …)이 남아 있어서, 더블클릭 그래프의
#   '값/임계 · ▲배수' 와 설명 패널의 '넘음' 표시가 예측기와 다르게 나왔다.
#   ★점수는 여기서 안 나온다 — 예측기가 CSV 에 적은 {FAB}_pts_* · area_score
#     를 그대로 읽는다(area_score). 이 값은 '화면에 임계를 어떻게 보여 주나' 뿐이다.
#   ★상세도끼리 어긋난 자리: M16A·M16B 의 SLA 건수증가 기준 — 2절 표는 +20,
#     5절 계산식은 26(M16A)·35(M16B). 계산식 쪽을 따랐다.
#   ★M16HUB R-D 의 STB 는 상세도에 '≥ 99.3' 이 남아 있지만, 2026-08 고객
#     요청(판정에서 제외)대로 record_only 로 둔다 — 확인되면 여기만 고친다.
WATCH: dict[str, dict[str, list[dict]]] = {
    "M16HUB": {
        ...
        "RA_sus": [{"amos": "M16HUB.QUE.TIME.AVGTOTALTIME1MIN", "csv": "M16HUB_ra",
                    "label": "반송시간 지속", "unit": "분", "op": ">=", "thr": 6.62}],
    ...
    "M14": {
        ...
        "RA_sus": [{"amos": "M14.QUE.LOAD.AVGLOADTIME1MIN", "csv": "M14_ra",
                    "label": "적재시간 지속", "unit": "분", "op": ">=", "thr": 2.96}],
        ...
        "RD": [{"amos": "M14.QUE.OHT.OHTUTIL", "csv": "M14_rd_oht",
                "label": "OHT 가동률", "unit": "%", "op": ">=", "thr": 93.18}],
    ...
    "M14B": {
        ...
        "RD": [{"amos": "M14B.QUE.OHT.OHTUTIL", "csv": "M14B_rd_oht",
                "label": "OHT 가동률", "unit": "%", "op": ">=", "thr": 96.6}],
...
RULE_KIND = {
    # ★한쪽으로만 읽으면 안 된다. 실제로 겪었다 — M14 가 70점까지 올라
    #   "사람이 내린 것이니 괜찮겠지" 하고 넘길 뻔했는데, 알고 보니 그 시각에
    #   CNV 고장이 나서 **사람이 용량을 내려 대응하던 중**이었다.
    #   용량변경은 '멀쩡하다' 는 신호가 아니라 '누가 손을 대고 있다' 는
    #   신호다. 계획 작업일 수도, 고장 대응일 수도 있다.
    "MAXCAPA": ("사람", "운영자가 용량을 임계 아래로 내린 것입니다 — **누가"
                        " 손을 대고 있다**는 뜻이지 멀쩡하다는 뜻이 아닙니다."
                        " 계획 정비일 수도, 방금 난 고장에 대응하는 중일 수도"
                        " 있습니다. 같은 시각에 다른 룰(특히 리프터 정체)이"
                        " 같이 켜졌으면 **고장 대응 쪽**을 먼저 의심하세요."
                        " 컬럼 하나당 10점이라 몇 개만 겹쳐도 크게 뜁니다."),
...
def explain(row: dict, fab: str, cfg: dict | None = None) -> dict:
    ...
    if human and human >= a["raw"] * 0.4:
        # ★같이 켜진 것이 있으면 '멀쩡하다' 로 읽으면 안 된다. 사람이 용량을
        #   내린 것은 **대응**일 수 있다 (실제로 CNV 고장 때 그랬다).
        with_fault = [p["label"] for p in parts
                      if p["kind"] != "사람" and p["pts"] > 0]
        if with_fault:
            notes.insert(0, "★이 점수의 {:.0f}% 가 운영자 용량변경에서 나왔는데,"
                            " **같은 시각에 {} 도 같이 켜졌습니다.** 사람이 용량을"
                            " 내린 것이 계획 작업이 아니라 **방금 난 고장에"
                            " 대응하는 중**일 수 있습니다 — 설비 이력을 함께"
                            " 확인하세요."
                         .format(100.0 * human / max(a["raw"], 1),
                                 "·".join(with_fault[:3])))
        else:
            notes.insert(0, "★이 점수의 {:.0f}% 가 **사람이 한 일**(운영자"
                            " 용량변경)에서 나왔고, 다른 룰은 하나도 안 켜졌습니다."
                            " 계획 작업·정비일 가능성이 큽니다 — 그래도 왜"
                            " 내렸는지는 확인하세요."
                         .format(100.0 * human / max(a["raw"], 1)))
...
def compare(rows: list[dict], at=None, cfg: dict | None = None,
    ...
    for f in fabs(cfg):
        ...
        #     다만 정책으로 매기면 뭐가 됐을지는 **알려만 준다**(아래 두 칸).
        #     등급 자체는 예전 그대로 예측기 값을 쓴다.
        if own and own.get("level"):
            g = dict(g, level=own["level"])
        fw, fd, fc = grade_cuts(fcfg)
        fl = str((own or {}).get("level") or "").strip()
        if fl:
            a["file_level"] = fl                      # 지금 쓰고 있는 등급
            byp = grade(r, fcfg)["level"]             # 정책으로 매기면?
            if byp != fl:
                a["policy_level"] = byp
                a["level_note"] = ("예측기 기준으로 '{}' 입니다. 지금 정책"
                                   "(경계 {})으로 매기면 '{}' 입니다 — 화면은"
                                   " 예측기 값을 씁니다."
                                   .format(fl, fw, byp))
        a.update({
