...
const Sessions = {
  async fetchList() {
    try {
      return await api("/api/harness/session/list");
    } catch {
      return { sessions: [] };
    }
  },
  ...
  _row(s) {
    ...
    row.querySelector(".session-del").addEventListener("click", async e => {
      ...
      try {
        const r = await fetch(`/api/harness/session/delete/${encodeURIComponent(s.session_id)}`, {
          method: "DELETE",
        });
  ...
  async load(sid) {
    try {
      const data = await api(`/api/harness/session/load/${encodeURIComponent(sid)}`);
