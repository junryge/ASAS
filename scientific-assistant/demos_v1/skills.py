...
SKILL_KEYWORDS = {
    ...
    # === M16 허브룸 반송 도메인 ===
    # 원본은 m16_hub_skills/ 이고, tools/sync_m16_skills.py 가 스킬 폴더로
    # 옮겨 심는다. 이 키워드가 없으면 '반송 정체' 를 물어도 gtars 같은
    # 엉뚱한 게 나온다 (실제로 그랬다).
    "m16-hub-overview": ["M16","허브룸","HUBROOM","반송","반송정체","구조","룰베이스","8영역","9룰","점수산식","등급"],
    "m16-hub-usage": ["실행방법","사용법","돌리는법","백테스트","실시간","hubroom_predictor","입력파일","출력파일"],
    "m16-hub-thresholds": ["임계값","thresholds","임계조정","튜닝","룰임계","민감도"],
    "m16-hub-interpret": ["결과해석","발동이벤트","사건단위","장애유형","정체","경계","위험","초위험","용어표준"],
    # === 가이드 ===
