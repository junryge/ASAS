...
async function send(){
  ...
  // Phase 2-A: 단일 API 모델 명시 선택 시 SSE 스트리밍
  // 제외 조건: auto / 병렬(env 2+) / GGUF / 첨부 파일 있음(서버측 파일 컨텍스트 미지원)
  ...
  if(_isStreamable){
    ...
    try{
      ...
      if(_ssError){
        ...
        let modelName = (_ssMeta && _ssMeta.model_used) || (envs[_se] ? envs[_se].name : _se);
        if(_ssMeta && _ssMeta.loaded_skills && _ssMeta.loaded_skills.length > 0){
          let mode = autoLoaded.length > 0 ? '\ud83e\udde0\uc790\ub3d9' : '\u2705\uc218\ub3d9';
          info = '\n['+mode+' 스킬: '+_ssMeta.loaded_skills.join(', ')+'] ['+modelName+'] ('+(_ssMeta.system_prompt_length ?? 0)+'\uc790)';
        } else {
          info = '\n['+modelName+']';
        }
        ...
        // markdown-mermaid-writing 또는 md-to-html 다운로드 버튼 (기존 로직 유지)
        const _mdSkillActive = ['markdown-mermaid-writing','md-to-html'].some(s => selSkills.includes(s) || autoLoadedSkills.includes(s));
        if(_mdSkillActive && !_ssFullText.includes('```markdown')){
          appendMdDownloadBar(_ssFullText, null);
        }
      } else {
