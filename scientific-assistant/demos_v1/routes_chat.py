...
try:
    ...
    _ctx_guard_demos = _ContextGuard(budget_tokens=32000, keep_recent=4, name="데모스", debug=True)
except Exception:
    class _NoGuardDemos:
...
def _builtin_hub_summary(headers, rows):
    ...
    try:
        ...
        out.append(
            f"[① 일일통계] 보고일자 {day} | 총 {n_total}분 | 정상 {n_total - n_cong}분 | 정체(50점↑) {n_cong}분"
        )
        out.append(
        ...
        out.append("")
        out.append(f"[③ 사건목록] (점수 50+ 구간 · 시각=최고점) — {len(incs)}건")
        if not incs:
            out.append("→ 금일 점수 50 이상 사건 없음 (정상 운영).")
        else:
