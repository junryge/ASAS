"""
demos_v1/hermes/builtin.py — 헤르메스 빌트인 스킬 팩 (Hermes 영감, 폐쇄망용 프롬프트 스킬)

개인 학습 스킬(skills.py)과 별개로, 어떤 사용자에게나 공통으로 쓰는
"작업 방식" 프롬프트 스킬. 질의 키워드에 맞으면 build_system_prompt 가 주입한다.
(외부망/실행 불필요 — 순수 프롬프트. execute_code 류는 제외.)
"""
...
def recall_builtin(query: str, top_k: int = 2) -> list[dict]:
    """질의 키워드에 맞는 빌트인 스킬 반환 (점수순 top_k)."""
    q = _tok(query)
    if not q:
        return []
    scored = []
