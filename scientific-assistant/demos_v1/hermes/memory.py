"""
demos_v1/hermes/memory.py — 장기 메모리 (선언적 사실)

- 두 저장소: MEMORY.md(환경/프로젝트 사실), USER.md(사용자 선호)
- 항목 구분자 "§"
- 액션: add / replace / remove / read  (replace/remove 는 짧은 고유 부분문자열 target)
- 인젝션 차단, 저장소당 6,000자 상한
- snapshot(): 시스템 프롬프트 주입용 텍스트
"""
...
def _path(user_id: str, store_name: str) -> str:
    fname = _STORES[store_name][0]
    return os.path.join(store.user_dir(user_id), fname)


def _valid_store(store_name: str) -> bool:
    return store_name in _STORES


def read_items(user_id: str, store_name: str) -> list[str]:
    if not _valid_store(store_name):
        return []
    raw = store.read_text(_path(user_id, store_name))
    items = [s.strip() for s in raw.split(store.ITEM_SEP)]
    return [s for s in items if s]


def _save_items(user_id: str, store_name: str, items: list[str]) -> None:
    body = ("\n" + store.ITEM_SEP + "\n").join(items)
...
def add(user_id: str, store_name: str, text: str) -> tuple[bool, str]:
    ...
    _save_items(user_id, store_name, new_items)
    return True, "저장됨"
...
def replace(user_id: str, store_name: str, target: str, text: str) -> tuple[bool, str]:
    ...
    items = read_items(user_id, store_name)
    matches = [i for i, it in enumerate(items) if target in it]
    if not matches:
        return False, f"대상 못 찾음: {target!r}"
    if len(matches) > 1:
        return False, f"대상이 모호함({len(matches)}개 일치): {target!r}"
    items[matches[0]] = text
    if len((store.ITEM_SEP).join(items)) > store.MAX_STORE_CHARS:
        return False, f"{store.MAX_STORE_CHARS}자 초과 — 거부"
    _save_items(user_id, store_name, items)
    return True, "교체됨"
...
def snapshot(user_id: str) -> str:
    """시스템 프롬프트 주입용. 비었으면 빈 문자열."""
    blocks = []
    for sname, (_fname, label) in _STORES.items():
