"""
demos_v1/hermes/curator.py — 큐레이터 (앱 시작 시 밀린 정리 따라잡기)

cron 대신 "앱 켤 때 마지막 실행일 확인 후 오늘 안 했으면 1회 실행".
- 30일 미사용 스킬 → state: stale
- 90일 미사용 스킬 → state: archived  (파일 보존, 자동 삭제 절대 금지)
- pinned 스킬은 면제
"""
...
from demos_v1.hermes import store, skills
...
def _run_for_user(user_id: str, now: float | None = None) -> dict:
    """한 사용자의 스킬 상태 전환. {staled, archived} 반환."""
...
def maybe_run(user_id: str) -> dict:
    """오늘 아직 안 돌았으면 1회 실행. 돌았으면 skip.
    Returns: {"ran": bool, "staled": int, "archived": int}
    """
