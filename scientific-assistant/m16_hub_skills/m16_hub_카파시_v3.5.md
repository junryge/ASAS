...
[수집기] → predict/M16A_HUBROOM_PR.csv (매분 ~265컬럼)
  ...
  - 하루 ≈ 1440행                     - 점수 60+ 이벤트만(1~100 정규화), 종료 후 기록
              ↓ (선택)
...
2. **점수 60 미만(경계 미만) 사건 제외** — 노이즈 차단(사건단위에만 적용. 발동이벤트는 매분 전부 기록).
...
> 심각도 표기(사건 등급 매핑, 등급/심각도 병기): 🟠경계→경계/주의(확인필요) / 🔴위험→위험/경고(모니터링 필요) / ⛔초위험→초위험/심각(조치필요)
