...
import com.skhynix.smartatlas.data.StageCommandRecordItem;
...
import com.skhynix.smartatlas.navi.DijkstraVhlRouteFind;
...
public class OhtMsgWorkerRunnable implements Runnable {
    ...
    public static final ConcurrentHashMap<String, OHT_UDP_TIB> tibrvMap = new ConcurrentHashMap<>(); 
    ...
    public void run() {
        ...
        if (tokens.length >= 2) {
            ...
            // Tibrv 발송 스위치 기능 추가
    		String requiredKey = fabId + ":" + mcpName;					
    ...
    public void _updateTibrvMap(String vhlName, String daemon, String subject, String message) {        
        String key = this.fabId + "." + this.mcpName + "." + vhlName;
        OHT_UDP_TIB _tib = new OHT_UDP_TIB(daemon, subject, message);
        
        tibrvMap.put(key, _tib);
    }
    ...
    private void _processHidInout(int currentHidId, Vhl vehicle, FunctionItem functionItem) {
        ...
        if (previousHidId != currentHidId) {
            ...
            String edgeKey = String.format("%03d:%03d:%s:%s:%s:%s:%s:%s:%s:%s:%s",
                    ...
                    vehicle.getFabId(), vhlName, eqpName, vhlCountLimit, vhlPrecaution, freeFlowSpeed, hidValue);
...
//        VHL_STATE vehicleState = vehicle.getState();
    ...
    public class OHT_UDP_TIB {
    	public String daemon;
    	public String subject;
    	public String message;
    	
    	public OHT_UDP_TIB(String daemon, String subject, String message)
    	{
    		this.daemon = daemon;
