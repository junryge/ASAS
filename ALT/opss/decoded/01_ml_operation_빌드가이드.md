...
> SmartAtlas 자바 Quartz 배치 2개(`HubroomTransPredictBatch.java`, `QTransferPredictBatch.java`)를
> Python ML 운영 시스템(`Prediction_ml.py` 마스터)으로 이식한 전 과정.
...
│
├── M14A_FAB/                     ▼ Q_TRANSFER 사이트 (반송 큐 예측 + 알람 11종)
│   ├── M14A_FAB_data_make.py         [기존] Logpresso→CSV (QTRANSFER.csv)
│   ├── V8.3.1_Q_TRANSFER_PREDICTOR_{10,15,25}m.py  [기존] 예측+저장
│   ├── m14a_config.json              [기존]
│   ├── api_key.txt                   [기존, 빈 파일 — 운영키 별도]
│   ├── QTransferPredictBatch.py      ★신규 (자바 이식: 통계+알람11종+적재)
│   └── qtransfer_alarm_config.json   ★신규 (임계치16/메시지27/쿼리11/Oracle 3)
│
...
| ④ 통합 적재 | `insertLogpressoData` | **★신규** `*PredictBatch.py` |
...
| `QTransferDashBoardItemBatch.java` | — | `insertLogpressoData` | `_insert_prediction_state()` 가 흡수 |
...
| `customQuery.xml` / `customQuery2.xml` (11종) | `queries` |
...
| 입력 | 쿼리/출처 | 용도 |
|---|---|---|
| **예측값** (LSTM/STATE/STATE_PER) | `predict_table`(test_table6) 최신 1건 | 알람 트리거 기준, 대시보드 상태 |
...
3) prediction 있으면:
   3-1) _insert_prediction_state()→ 대시보드 테이블(test_table7)에 상태 적재
...
|---|---|---|---|---|
| 통계+예측+알람 | `insert_table` | **test_table5** | test_currentjob_predict | 행 3종 |
| 대시보드 상태 | `dashboard_table` | **test_table7** | qtransfer_dashboard | STATE 한글(정상/주의/심각)+확률 |
| (예측 스크립트가 씀) | `predict_table` | **test_table6** | ATLAS_TS_PREDICT | LSTM/STATE/STATE_PER |

저장 방식 — Logpresso httpexport `import`:
```
...
"M14A_FAB": { "scripts": [
    ...
    "QTransferPredictBatch.py --once",         # ③④ ★신규
]},
```
...
| 역할 | 구 운영 | 현재 테스트 | config 키 |
|---|---|---|---|
| Hubroom WARN | test_hubroom_predict | **test_table** | hubroom `insert_table` |
| QTransfer 통계+알람 | test_currentjob_predict | **test_table5** | qtransfer `insert_table` |
| QTransfer 예측 | ATLAS_TS_PREDICT | **test_table6** | qtransfer `predict_table` |
| QTransfer 대시보드 | qtransfer_dashboard | **test_table7** | qtransfer `dashboard_table` |

> 운영 전환 = config 의 위 4개 값만 원복. 코드 수정 불필요(테이블명 전부 config 변수에서 읽음).
