...
model_dir = os.path.join(script_dir, 'model_15min', 'xgboost_Numerical_V8_15min.pkl')
...
def realtime_prediction():
    try:
        ...
        models = model_dict['models']  # [MIN, MAX, AVG] 3개 모델
    except:
    ...
    prediction_time = seq_end + timedelta(minutes=15)
    ...
    return {
        ...
        'pred_max': pred_max,
        'pred_avg': pred_avg,
        'change_min': pred_change_min,
        'change_max': pred_change_max,
        'change_avg': pred_change_avg,
        'status': status,
...
# 모델 메타 (config.json의 models.numerical_15m)
_model_cfg = _cfg["models"]["numerical_15m"]
...
def predict_realtime():
    ...
    if result is None:
        ...
        surge_warning = int(result['pred_max'] >= 280)
        extreme_warning = int(result['pred_max'] >= 300)
        ...
        row = {
            ...
            "PREDICT_MIN": round(result['pred_min'], 2),
            "PREDICTVAL": round(result['pred_max'], 2),
            "PREDICT_AVG": round(result['pred_avg'], 2),
            "CHANGE_MIN": round(result['change_min'], 2),
            "CHANGE_MAX": round(result['change_max'], 2),
            "CHANGE_AVG": round(result['change_avg'], 2),
            "SURGE_WARNING": surge_warning,
            "JUDGEVAL": extreme_warning,
