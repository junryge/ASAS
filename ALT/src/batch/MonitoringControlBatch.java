public class MonitoringControlBatch implements Job{
		...
		if (Util.isCurrentIC()) {
			...
			new Thread("MonitoringControlBatch") {
				...
				public void run() {
					...
					_processUdpMessageMonitoring(switchMap.get("CMN"));
				}
			}.start();
		}
	}
	 ...
	 */
	private void _processUdpMessageMonitoring(FunctionItem functionItem) {
		if (functionItem.isUseUdpMessageMonitoring()) {
			logger.info("... process of `UdpMessageMonitoring` has started in `MonitoringControlBatch`");

			long timer = System.currentTimeMillis();

			this._processUdpMessageMonitoringHandler();
	...
	private void _processUdpMessageMonitoringHandler() {
		...
		final BlockingQueue<Msg> recordQueue = DataService.getInstance().recordQueue;
		...
		recordQueue.drainTo(records);    // 기존의 recordQueue 값은 비워줌
