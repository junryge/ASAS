...
public class Util {
	...
	public static boolean isCurrentIC() {
		if (DataService.getInstance() != null && DataService.getInstance().getInitialized()) {
			...
			return (fabIds.contains("M14A") || fabIds.contains("M14B") || fabIds.contains("M16A") || fabIds.contains("M16B") || fabIds.contains("ICPKT") || fabIds.contains("ICPNT")
					|| fabIds.contains("R3")
					|| fabIds.contains("TSV")
					|| fabIds.contains("DWT")
					|| fabIds.contains("WLP")
					|| fabIds.contains("ICPKG"));
		}
...
// ---------------------------------------------------------------------------------------------------------------------
	...
	private static boolean _connectFtpServer(
		...
		do {
			if (StringUtils.isEmpty(hostName) == true) {
				logger.info("... Skip to connect FTP server [fab: {} | mcp: {}]", fabId, mcpName);
				break;
			}
	...
	public static void reflectSwitch(Properties properties) throws NullPointerException {
		...
		for (FabProperties fabProperties : fabPropertiesMap.values()) {
			...
			// #3 Conveyor(CNV_INOUT) switch 기능 구성
			for(Map.Entry<String, String> entry : fabProperties.getConveyorToApiUrl().entrySet()) {
				List<String> context = _setFunctionItem(fabId, entry.getKey(), properties);
				logs.addAll(context);
			}			
		}		
		// #4 AMP
		String ampUrl = DataService.getInstance().getAmpUrl();
		if(!StringUtils.isEmpty(ampUrl))
		{
			for (FabProperties fabProperties : fabPropertiesMap.values()) {
				String fabId = fabProperties.getFabId();
				
				List<String> context = _setFunctionItem(fabId, "AMP", properties);
				logs.addAll(context);			
			}	
		}
