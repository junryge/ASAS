...
import com.skhynix.smartatlas.db.mybatis.MybatisQueryHandler;
import com.skhynix.smartatlas.queryformat.LogpressoCommonFilterQuery;
import com.skhynix.smartatlas.queryformat.LogpressoMcslogQuery;
import com.skhynix.smartatlas.queryformat.MongodbCommonFilterQuery;
...
import com.skhynix.smartatlas.queryformat.util.LogpressoConditionUtil;
import com.skhynix.smartatlas.util.AsyncUtil;
import com.skhynix.smartatlas.util.JsonUtil;
import com.skhynix.smartatlas.util.MemoryTailerUtil;
import com.skhynix.smartatlas.util.QueryUtil;
import com.skhynix.smartatlas.util.ReflectionUtil;
import com.skhynix.smartatlas.util.RemoteCommandUtil;
import com.skhynix.smartatlas.util.SecurityUtil;
import com.skhynix.smartatlas.util.SecurityUtil.AccessLevel;
	...
	public Map<String, Object> getTotalLogListForMongodb(final String fabSite, List<String> fabs, List<String> levels,
		...
		try {
			...
			boolean isViewTable = levels.stream()
					.anyMatch(l -> StringUtils.equalsAny(l.toUpperCase(), "WELL", "WARN", "ERROR", "FATAL"));
			boolean isDataTable = levels.stream()
					.anyMatch(l -> StringUtils.equalsAny(l.toUpperCase(), "DEBUG", "INFO", "FINE"));
			
			for (String fab : fabs) {
				String suffix = MongodbMcslogQuery.getSuffix(fab);
				String isViewTableName = isViewTable ? QueryUtil.getTsViewPartitionCollection("ts_data_view_" + suffix,
						QueryUtil.convertToDate(from), QueryUtil.convertToDate(to)) : "";
				String isDataTableName = isDataTable ? "ts_data_" + suffix : "";
				
				for (String collection : new String[] { isViewTableName, isDataTableName }) {
	...
	public Map<String, Object> getRawLogListForMongodb(final String fabSite, List<String> fabs, List<String> levels,
		...
		try {
			...
			for (String fab : fabs) {
				...
				if (logs.contains("TS")) {
					collections.add("ts_raw_" + suffix);
				}
