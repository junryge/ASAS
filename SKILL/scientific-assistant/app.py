...
8. **기간 지정은 반드시 `duration=` 문법만 사용하세요. `from=`/`to=` 문법은 절대 사용하지 마세요.** httpexport API에서 지원하지 않습니다. 예: 최근 1시간 → `duration=1h`, 최근 7일 → `duration=7d`, 최근 30일 → `duration=30d`, 3월 1일~25일(약 25일) → `duration=25d`
...
12. **`| fields`는 사용자가 특정 필드를 요청한 경우에만 사용하세요.** 컬럼 정보를 모르는 테이블에 필드를 임의로 추가하지 마세요. 필드 지정 없이 `table ... TABLE_NAME | limit 5`로 전체 컬럼을 조회하면 됩니다.
13. **`| sort`도 사용자가 정렬을 요청한 경우에만 사용하세요.** 컬럼명을 모르면서 임의로 sort를 추가하지 마세요.
