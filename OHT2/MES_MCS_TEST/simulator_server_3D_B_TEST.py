...
async def dispatch_vehicle(request: dict):
    ...
    Request:
        requestId: Transport 요청 ID
        carrierId: Carrier ID (FOUP)
        fromStation: 출발 Station
        toStation: 도착 Station
        priority: 우선순위
    ...
    from_station = request.get("fromStation")
    to_station = request.get("toStation")
    ...
    active_transports[request_id] = transport

    # 차량에 목적지 설정
    vehicle.transportRequestId = request_id
    vehicle.carrierId = carrier_id
    vehicle.targetStation = from_station  # 먼저 픽업 위치로
    vehicle.finalStation = to_station
    vehicle.transportPhase = "TO_PICKUP"  # TO_PICKUP -> PICKING -> TO_DELIVERY -> COMPLETE

    print(f"[MCS] 배차 완료: {vehicle.vehicleId} → Station {from_station}")
...
def find_nearest_idle_vehicle(target_station: int):
    ...
    if not engine or not engine.vehicles:
        return None

    # Station 좌표 찾기
    target_node = None
    for station in engine.stations.values():
        if station.stationId == target_station:
            target_node = station.nodeAddress
            break

    if not target_node:
        # Station을 찾지 못하면 임의의 노드로
        target_node = target_station

    target_pos = engine.nodes.get(target_node)
    if not target_pos:
        return None

    # Idle 차량 중 가장 가까운 것 선택
    best_vehicle = None
    best_distance = float('inf')

    for v in engine.vehicles.values():
        # 이미 Transport 중인 차량 제외
        if hasattr(v, 'transportRequestId') and v.transportRequestId:
            continue

        # 정지 상태가 아닌 차량도 고려 (RUN 상태)
        dist = ((v.x - target_pos.x) ** 2 + (v.y - target_pos.y) ** 2) ** 0.5
...
async def update_mcs_status(request_id: str, status: str, vehicle_id: str = None):
    """MCS에 Transport 상태 업데이트"""
    try:
        async with httpx.AsyncClient() as client:
            await client.post(
                f"{MCS_URL}/api/transport/{request_id}/status",
                json={"status": status, "vehicleId": vehicle_id},
                timeout=3.0
            )
    except Exception as e:
        print(f"[MCS] 상태 업데이트 실패: {e}")

@app.get("/api/transports")
async def get_active_transports():
    """활성 Transport 목록 조회"""
    return list(active_transports.values())
...
if __name__ == "__main__":
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    print("=" * 50)
    print("OHT Simulator (MCS Integration Test)")
    print("=" * 50)
    print("URL: http://localhost:10003")
    print("MCS: http://localhost:10011")
    print("=" * 50)
