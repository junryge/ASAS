(function ($) {
   ...
   */
  function RemoteModel(_url,_param) {
    // private
    var PAGESIZE = 200;
    var data = {length: 0};
    var searchstr = "";
...
//    	  url += ("?page=" + (fromPage * PAGESIZE) + "&limit=" + (((toPage - fromPage) * PAGESIZE) + PAGESIZE));
    	  param.page = fromPage;
    	  param.rows = PAGESIZE;
    	  //url += ("?page=" + fromPage + "&rows=" + PAGESIZE);
      }
...
//        });
    ...
    function onSuccess(resp) {
      var from = req.fromPage * PAGESIZE, to = from + resp.rows.length;
      data.length = Math.min(parseInt(resp.total),1000000); // limitation of the API
