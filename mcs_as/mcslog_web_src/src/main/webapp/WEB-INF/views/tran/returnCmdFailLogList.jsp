...
<div class="contents_wrap" id="body_<c:out value="${param.uuid }" />">
         ...
         <div class="loc_info_basic">
             ...
             <span class="loc_info_ico loc_info_ico_arr_depth"></span>
             <span class="location_box">
                 <a href="javascript:movePage('<c:url value="/tran/returnCmdFailLogList.do" />')" class="location"><spring:message code="site.returnCmdFailLogList" text="default text" /></a>
             </span>
         </div>
         <!-- //Location Information -->
         <!-- Page Title -->
         <table class="page_tit">
             <tr>
                 <td class="tit_area">
                     <div class="tit"><spring:message code="site.returnCmdFailLogList" text="default text" /></div>
                 </td>
             </tr>
         </table>
         <!-- //Page Title -->
                                     ...
                                     <div class="tab_contents_wrap" rel="tree_set_tab">
                                         <div class="tab_contents" id="tree_set_tab_contents1">
					                            ...
					                            <tbody>
					                                ...
					                                <tr>
					                                    <td class="condition_t_data" colspan="3">
					                                    	<input type="text" id="carrier" name="carrier" value="" style="width:210px" />
					                                    </td>
					                                 </tr>
					                               </tbody>
					                               </table>
         									...
         									<div class="condition_area">
                                                 <table class="condition_table" summary="<spring:message code="site.common.filter" text="default text" />">
					                            <caption><spring:message code="site.common.filter" text="default text" /></caption>
					                            <tbody id="tTransportCmdId">
					                                <tr>
					                                    <td class="condition_t_head_top" colspan="3">
					                                    	<i class="minus square icon"></i>
					                                    	<span>TansportCommand ID</span>
					                                    </td>
					                                </tr>
					                                <tr>
					                                    <td class="condition_t_data" colspan="3">
					                                    	<input type="text" id="transportCmdId" name="transportCmdId" value="" style="width:210px" />
					                                    </td>
					                                 </tr>
					                               </tbody>
					                               </table>
         									...
         									<div class="condition_area">
					                            ...
					                            <tbody id="reasonText">
					                                ...
					                                <tr>
					                                    <td class="condition_t_data" colspan="3">
					                                    	<input type="text" id="reason" name="reason" value="" style="width:210px" />
			                                                    <div id="reasonBtn" class="mini ui primary button" style="width:45px;float: right;margin-left: 4px" title="reason">
         									...
         									<div class="condition_area">
					                            ...
					                            <tbody>
					                                ...
					                                <tr>
					                                    ...
					                                    <td class="condition_t_data" >
					                                    	<input type="text" class="inp_date" id="fromDt<c:out value="${param.uuid }" />" name="fromDt" value="" />
					                                ...
					                                <tr>
					                                    ...
					                                    <td class="condition_t_data" >
					                                    	<input type="text" class="inp_date" id="toDt<c:out value="${param.uuid }" />" name="toDt" value="" />
...
<script type="text/javascript">
	$content = $("#body_${param.uuid }");
 	$(document).ready(function(){
			...
			/* $content.find(":checkbox[name^=fromFab]").click(function(){
				var _fabSite = $content.find('input[name="rdoFabSite"]:checked').val();
				var val = $(this).val();
				if(val == "ALL"){ // ALL 체크시, 다른 체크 박스 해제
					$content.find(":checkbox[name^=fromFab]:gt(0)").prop("checked",false);
					$content.find(":checkbox[name^=fromFab]:eq(0)").prop("checked",true);
				}else{  // 다른 체크박스 체크시 , ALL 체크박스 해제
					var tmpChk = 0;
					$content.find(":checkbox[name^=fromFab]:checked").each(function(){
						tmpChk += 1;
					});
					if( tmpChk > 0){
						$content.find(":checkbox[name^=fromFab]:eq(0)").prop("checked",false);
					}else{
						$content.find(":checkbox[name^=fromFab]:eq(0)").prop("checked",true);
					}
				}
				
				getMachineTypeFromFab(_fabSite,"fromFab" , "singleFilterChkBoxFromMachineType");				
				getAreaFromFab(_fabSite,"fromFab");
				getBayFromArea(_fabSite,"fromAreaName", "fromFab");
			...
			/* $content.find(":checkbox[name^=toFab]").click(function(){
				var _fabSite = $content.find('input[name="rdoFabSite"]:checked').val();
				var val = $(this).val();
				if(val == "ALL"){ // ALL 체크시, 다른 체크 박스 해제
					$content.find(":checkbox[name^=toFab]:gt(0)").prop("checked",false);
					$content.find(":checkbox[name^=toFab]:eq(0)").prop("checked",true);
				}else{  // 다른 체크박스 체크시 , ALL 체크박스 해제
					var tmpChk = 0;
					$content.find(":checkbox[name^=toFab]:checked").each(function(){
						tmpChk += 1;
					});
					if( tmpChk > 0){
						$content.find(":checkbox[name^=toFab]:eq(0)").prop("checked",false);
					}else{
						$content.find(":checkbox[name^=toFab]:eq(0)").prop("checked",true);
					}
				}
				
				getMachineTypeToFab(_fabSite,"toFab" , "singleFilterChkBoxToMachineType");				
				getAreaFromFab(_fabSite,"toFab");
				getBayFromArea(_fabSite,"toAreaName", "toFab");
			...
			// icon 버튼 클릭
			$content.find("#reasonBtn").click(function(){
				var url = "<c:url value='/tran/pop/reasonPop.do' />";
				openPopup(url , 820 , 680,function(data){
					$("#reason").val(data);
				});
			});
	...
	function drawGrid<c:out value="${param.uuid }" />(){
		var columns = [
		  ...
		  {id: "FROM FAB", name: "FROM FAB", field: "SOURCEFAB", minWidth: 80, sortable: true},
		  {id: "FROM MACHINE", name: "FROM MACHINE", field: "SOURCEMACHINENAME", minWidth: 100, sortable: true},
		  {id: "FROM MACHINETYPE", name: "FROM MACHINETYPE", field: "SOURCEMACHINETYPE2", minWidth: 100, sortable: true},
		  {id: "FROM TYPE", name: "FROM TYPE", field: "SOURCETYPE", minWidth: 60, sortable: true},
		  {id: "FROM UNIT", name: "FROM UNIT", field: "SOURCEUNITNAME", minWidth: 100, sortable: true},
		  {id: "TO FAB", name: "TO FAB", field: "DESTFAB", minWidth: 80, sortable: true},
		  ...
		  {id: "REASON", name: "REASON", field: "REASON", minWidth: 200, sortable: true},
	...
	function getLogList<c:out value="${param.uuid }" />(page){
		...
		var url = "<c:url value="/tran/ajax/getReturnCmdFailLogList.do" />";
