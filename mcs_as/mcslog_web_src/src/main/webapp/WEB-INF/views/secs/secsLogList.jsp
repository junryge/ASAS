...
<%@ include file="/WEB-INF/views/common-taglib.jspf"%>
    <div class="contents_wrap" id="body_<c:out value="${param.uuid }" />">
        ...
        <div class="loc_info_basic">
            ...
            <span class="location_box">
                <a href="#" class="location"><spring:message code="site.secsLogList" text="default text" /></a>
            </span>
            <span class="loc_info_ico loc_info_ico_arr_depth"></span>
            <span class="location_box">
                <a href="#" class="location"><spring:message code="site.secsLogList" text="default text" /></a>
        ...
        <table class="page_tit">
            <tr>
                <td class="tit_area">
                    <div class="tit"><spring:message code="site.secsLogList" text="default text" /></div>
                </td>
            </tr>
        </table>
        <!-- //Page Title -->
                    ...
                    <div class="lay_item_box">
						                            ...
						                            <tbody>
						                                ...
						                                <tr>
						                                    ...
						                                    <td class="condition_t_data" id="tdFab">
						                                    	<input type="checkbox" class="jqForm" id="secsFab1" name="secsFab1" value="ALL"><label for="secsFab1">ALL</label><BR>
		                                            			...
		                                            			<c:forEach  items="${fabs}" var="fab" varStatus="status">
																	...
																	<input type="checkbox" class="jqForm" id="secsFab<c:out value="${num}"/>" name="secsFab<c:out value="${num}"/>" <c:if test="${isVal eq 'T'}">checked="checked"</c:if> value="<c:out value="${fab}"/>" ><label for="secsFab<c:out value="${num}"/>"><c:out value="${fab}"/></label><BR>
		                                            			</c:forEach>
						                                    </td>     
						                                </tr>
						                            </tbody>
					                            ...
					                            <tbody>
					                                ...
					                                <tr>
					                                    ...
					                                    <%-- <td class="condition_t_data">
					                                         ...
					                                         <input type="checkbox" class="jqForm" id="level2" name="level2" <c:if test="${param.level2 == 'TIME' }" >checked="checked"</c:if> value="TIME" ><label for="level2">TIME</label><BR>
					                                         ...
					                                         <input type="checkbox" class="jqForm" id="level4" name="level4" <c:if test="${param.level4 == 'WARN' }" >checked="checked"</c:if> value="WARN" ><label for="level4">WARN</label><BR>
					                                    </td>
					                                    <td class="condition_t_data">
					                                         <input type="checkbox" class="jqForm" id="level5" name="level5"  checked value="RECV" ><label for="level5">RECV</label><BR>
					                                         <input type="checkbox" class="jqForm" id="level6" name="level6"  checked value="SEND" ><label for="level6">SEND</label><BR>
					                                    </td> --%>
					                                    <td class="condition_t_data">
           								...
           								<div class="srch_type01">
        									<div class="condition_area">
					                            ...
					                            <tbody>
					                                ...
					                                <tr>
					                                	<th scope="col" class="condition_t_head">SECSII</th>
					                                    <td class="condition_t_data">
					                                         <select class="secsName" id="secsIInameList" name="secsIInameList" style="width:158px;margin-bottom: 5px">
					                                         	<option value='ALL' selected='selected' >ALL</option>
					                                        </select>
					                                        <input type="text" id="secsIIname" name="secs" title="Multi Condition Search &#10;ex) SECS1, SECS2, SECS3" value="" />
					                                    </td>
					                                </tr>
					                                <tr>
					                                ...
					                                <tr>
					                                	...
					                                	<input type="hidden" name="secsTextConditionCheckBox" value="and"/>
					                                		<input type="checkbox" class="jqForm" id="secsTextConditionCheckBox" name="secsTextConditionCheckBox" checked value="or" style="margin-left: 50px;"><label id="AndOr_selectLabel">or</label>
					                                	</th>
					                                    <td class="condition_t_data">
					                                         <input type="text" id="text" name="text" title="Multi Condition Search &#10;ex) 4PDN6591, 4AFZ41-418, 4PTI4101_3" value="<c:out value="${param.text }" />" />
					                                    </td>
					                                </tr>
					                            </tbody>
					                        </table>
                        ...
                        <div class="lay_item_right">
                            ...
                            <div id="grid_container<c:out value="${param.uuid }" />">
                                <div id="list<c:out value="${param.uuid }" />" class="gridForResize_secs" style="width:100%;height:565px; background: white; outline: 0; border: 1px solid gray;"></div>
                            	<c:import url="/WEB-INF/views/common/slickGridPager.jsp" charEncoding="utf-8" />
                    		</div>
                    		<!-- //gridDetail -->
                 </div>
                 <!-- 20200403 hgJeon popup 창 대체로 인해 주석처리 -->
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//secsLog 화면 resize 1초 이벤트
 		function resizedwSecs(){
			console.log("resizedwSecs!! ");
		...
		window.onresize = function(){
		  clearTimeout(doit);
		  doit = setTimeout(resizedwSecs, 1000);
		};
		...
		// 200507 hgJeon Fab 기준정보에 따른 Naming 변경
 		...
 		case 'M14' :
 			...
 			$("label[for = 'secsFab2' ]").text('M14A')
 			$("label[for = 'secsFab3' ]").text('M16')
 			$("#secsFab3").hide();
 			$("label[for = 'secsFab3' ]").hide();
 			break;
 		case 'M15' :
			{console.log('FAB 확인 : ' , FabCode);}
			$("label[for = 'secsFab2' ]").text('M15A')
 		...
 		case 'M11' :
			{console.log('FAB 확인 : ' , FabCode);}
			$("label[for = 'secsFab2' ]").text('M11A')
 			$("label[for = 'secsFab3' ]").text('M11B')
			break;
 		case 'C2' :
			{console.log('FAB 확인 : ' , FabCode);}
			$("label[for = 'secsFab2' ]").text('C2')
 			$("label[for = 'secsFab3' ]").text('C2F')
			break;
 		case 'IC' :
			{console.log('FAB 확인 : ' , FabCode);}
			$("label[for = 'secsFab2' ]").text('M14A')
		...
		$content.find('input[name="rdoFabSite"]').change(function() {			
			...
			getFabFromFabSite("secs", _fabSite, "secsFab", "tdFab");
			//getMachineTypeFromFab(_fabSite,"secsFab", "machineType", "singleFilterChkBoxFab");				
			//getAreaFromFab(_fabSite,"secsFab", "areaName");
			//getBayFromArea(_fabSite,"secsFab", "areaName", "bayName");
			//getMachineNameList(_fabSite,"secsFab", "areaName","bayName","machineType","machineName1");
		});
		
		// 20180615 FAB 클릭 이벤트
		$content.on("click", ":checkbox[name^=secsFab]", function(){
			var val = $(this).val();
			if(val == "ALL"){ // ALL 체크시, 다른 체크 박스 해제
				$content.find(":checkbox[name^=secsFab]:gt(0)").prop("checked",false);
				$content.find(":checkbox[name^=secsFab]:eq(0)").prop("checked",true);
			}else{  // 다른 체크박스 체크시 , ALL 체크박스 해제
				var tmpChk = 0;
				$content.find(":checkbox[name^=secsFab]:checked").each(function(){
					tmpChk += 1;
				});
				if( tmpChk > 0){
					$content.find(":checkbox[name^=secsFab]:eq(0)").prop("checked",false);
				}else{
					$content.find(":checkbox[name^=secsFab]:eq(0)").prop("checked",true);
				}
			}
			getSecsFabList("secsFab");
		});
		...
		$content.find("#cancelBtn").click(function(){
			...
			var url = "<c:url value='/ei/ajax/getSecsQueryStop.do' />"; 
			$.ajax({
	            url: url,
	            type:'post',
	            //data:param,
  	...
  	// secsIIname 키 입력 이벤트
	$("#secsIIname").keyup(function(e){
		if($(this).val() == ""){ // ALL
			$content.find("#secsIInameList").val("").prop("selected", true);
		}
		else{ // 직접입력
			$content.find("#secsIInameList").val("write").prop("selected", true);
		}
	});
  	
	// secsIIname 셀렉트 값 변경
	/* $("#secsIInameList").change(function(e){
		var val = $(this).val();
		if(val == "write") val = "";
		$content.find("#secsIIname").val(val);
	}); */
	$content.find("#secsIInameList").change(function(){
		var val = $(this).val();
		if(val == "write") val = "";
		$content.find("#secsIIname").val(val);
	});
	...
	function init<c:out value="${param.uuid }" />(){
		...
		//secsList 적용
		getSecsNameList($content.find('input[name="rdoFabSite"]:checked').val());
	}
	...
	function drawGrid<c:out value="${param.uuid }" />(){
		var columns = [
			  ...
			  {id: "TIME", name: "TIME", field: "TIME_EX", width: 80, minWidth: 60, cssClass: "cell-title", sortable: true},
			  {id: "SECSII", name: "SECSII", field: "SECSII" , width: 40 , minWidth: 30, sortable: true},
			  {id: "HOST", name: "HOST", field: "HOST" , width: 40 , minWidth: 30, sortable: true},
			  {id: "LEVEL", name: "LEVEL", field: "LEVEL" , width: 24 , minWidth: 20, sortable: true},
			  {id: "S/F", name: "S/F", field: "S/F" , width: 40 , minWidth: 30, sortable: true},
			  {id: "SB", name: "SB", field: "SB" , width: 40 , minWidth: 30, sortable: true},
			  {id: "NAME", name: "NAME", field: "NAME" , width: 80 , minWidth: 60, sortable: true},
			  {id: "DATA", name: "DATA", field: "DATA", width: 360, minWidth: 200, sortable: true},
			...
			grid<c:out value="${param.uuid }" />.onActiveCellChanged.subscribe(function (e, args) {				
			    ...
			    getDetailInfo(row); // 상세조회 정보 보임
	     		//$content.find("#SECS_II").text(row.TEXT);
  				...
  				var url = "<c:url value='/ei/pop/textAreaPop.do' />";
				...
				/* if(textDetailPopup == null || (textDetailPopup.closed)) {
					...
					textDetailPopup.detailTextFindFocus(secsPopOption);	// child window function
				} */
			});
	...
	function setSearchOption<c:out value="${param.uuid }" />(colName , colValue){
		...
		switch(colName) {
			case "HOST":
		        ...
		        break;
		    case "SECSII":
		    	$content.find("#secsIInameList").val(colValue).prop("selected", true);
		    	$content.find("#secsIIname").val(colValue);
		        break;
		    default:
		}
	}
	
	function getSecsFabList(secsFab){
		var secs = $content.find(":checkbox[name^="+secsFab+"]:checked").map(function(){return $(this).val(); }).get();
		var param = {"selectFab":secs};
			var urlSecsName = "filter/ajax/getSecsFabList.do";	 
			$.ajax({
	            url: urlSecsName,
	            type:'post',
	            data: param,
	            ...
	            success:function(data){	            	
	            	var $select = $content.find(".secsName");
	            	$select.empty();
	            	$select.append("<option value='ALL' selected='selected'>ALL</option>");
	            	for(var i in data.list){ // machine name list  셀렉트 생성
						var $opt = $("<option value=''></option>");  
						$opt.attr("value" , data.list[i].SECSII);
						$opt.text(data.list[i].SECSII);
						$select.append($opt);
	            	}
	            	$content.find(".secsName").append("<option value='write'>직접입력</option>");
	            }
		    });
	}
	
	function getSecsNameList(fabSite){
		setTimeout(function(){
			var urlSecsName = "filter/ajax/getSecsList.do";	 
			...
			$.ajax({
	            ...
	            success:function(data){
	            	...
	            	$.each(result, function(index, value){
	            		$content.find(".secsName").append(""+
	            		"<option value='"+value.SECSII+"'>"+value.SECSII+"</option>"); 
	            	}); 
	            	$content.find(".secsName").append("<option value='write'>직접입력</option>"); 
	            }
		    });
	...
	$content.find("#secsTextConditionCheckBox").click(function(){
		var textCondition = document.getElementById("secsTextConditionCheckBox");
