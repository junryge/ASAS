...
@Controller
public class TranJobFailController {
	protected Log log = LogFactory.getLog(TranJobFailController.class);
	@Resource(name = "jobFailService")
	private TranService jobFailService;
	
//	@Resource(name = "totalService")
//	private TotalService totService;
	
	@RequestMapping(value = "tran/returnJobFailLogList")
	public ModelAndView tranJobFail(@ModelAttribute TranJobFailVo param, HttpServletRequest request) throws Exception {
...
//		param.setToFab(Common.getBasicFabList("tran",Common.sFAB_SITE));
		...
		mav.setViewName("tran/returnJobFailLogList");
		return mav;
	}
	
	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "tran/ajax/getReturnJobFailLogList")
	public ModelAndView getReturnJobFailLogList(@ModelAttribute TranJobFailVo param, HttpServletRequest request)	throws Exception {
		log.info("getReturnJobFailLogList : Start!!");
		ModelAndView mav = new ModelAndView();
...
//		param.setFabSite(sFabSite);
		...
		if (request.getParameter("toMachineType1") != null && !request.getParameter("toMachineType1").equals("")
				&& request.getParameter("toMachineType1").equals(Common.sALL)) {
			toMachineType.clear();
		}else{
		...
		List list = jobFailService.getDataList(param);
