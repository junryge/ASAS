 ...
 * @FileName : TranJobHistoryServiceImpl.java
 * @작성일 : 2017. 3.20.
 * @작성자 : 최명수
 * @프로그램 설명 : 반송 이력 조회 서비스
 */
@Service("tranCmdHistoryService")
public class TranCmdHistoryServiceImpl implements TranService {
	protected Log log = LogFactory.getLog(TranCmdHistoryServiceImpl.class);
	...
	/**
	 * @Method Name : getTranCmdHistoryQueryParser
	 * @작성일 : 2017. 3. 20.
	 * @작성자 : 최명수
	 * @param :
	 * @Method 설명 : 반송 이력 조회
	 ...
	 * @throws Exception
	 */
	public String getTranCmdHistoryQueryParser(TranVo tranVo){
		if (tranVo == null) {
			return null;
		} // null exception
		...
		//TransportCommandId
		if(tranVo.getTransportCommandId()!=null&&!tranVo.getTransportCommandId().equals("")){
			sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
					+ Common.sTRANSPORTCOMMANDID + Common.sEquals
					+ Common.sDoubleQuotation + tranVo.getTransportCommandId() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		}		
		//State
		if (tranVo.getState()!=null && tranVo.getState().size() > 0) {
			List<String> state = tranVo.getState();
			if(state.size()==1){
				if(state.get(0).equals(Common.sALL)){
				}else{
					sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis  
							+ Common.sLeftParenthesis + Common.sSTATE + Common.sEquals
							+ Common.sDoubleQuotation + state.get(0) 
				...
				for (int i=0;i<state.size();i++) {
					if (i==0) {
						if(state.get(0).equals(Common.sALL)){
							...
							sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis  
									+ Common.sLeftParenthesis + Common.sSTATE + Common.sEquals
									+ Common.sDoubleQuotation + state.get(i) 
									+ Common.sDoubleQuotation + Common.sRightParenthesis);
						}
					}else if(i==(state.size()-1)){
						sQuery.append(Common.sOr + Common.sLeftParenthesis + Common.sSTATE + Common.sEquals
								+ Common.sDoubleQuotation + state.get(i) 
								+ Common.sDoubleQuotation + Common.sRightParenthesis
		...
		//transportUnit
		if (tranVo.getTransportUnit()!=null && !tranVo.getTransportUnit().equals("")) {
			if(tranVo.getTransportUnit().contains(Common.sUnderbar)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getTransportUnit().split(Common.sUnderbar);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
								+ Common.sLeftParenthesis + Common.sTRANSPORTUNITNAME + Common.sEquals
						...
						tmpQuery.append(
								...
								+ Common.sTRANSPORTUNITNAME + Common.sEquals
								+ Common.sDoubleQuotation + unitNameArray[i]
								+ Common.sDoubleQuotation 
			...
			}else if(!tranVo.getTransportUnit().contains(Common.sUnderbar)&&tranVo.getTransportUnit().contains(Common.sMinus)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getTransportUnit().split(Common.sMinus);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
				...
				sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
						+ Common.sTRANSPORTUNITNAME + Common.sEquals
						+ Common.sDoubleQuotation + tranVo.getTransportUnit() 
						+ Common.sDoubleQuotation + Common.sRightParenthesis);
			}
		}
		
		//fromUnit
		if (tranVo.getFromUnit()!=null && !tranVo.getFromUnit().equals("")) {
			if(tranVo.getFromUnit().contains(Common.sUnderbar)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getFromUnit().split(Common.sUnderbar);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
								+ Common.sLeftParenthesis + Common.sSOURCEUNITNAME + Common.sEquals
								+ Common.sDoubleQuotation + unitNameArray[i]
								+ Common.sDoubleQuotation 
								+ Common.sRightParenthesis + Common.sAnd);
					}else if(i==(unitNameArray.length-1)){
						tmpQuery.append(
								Common.sLeftParenthesis
								+ Common.sSOURCEUNITNAME + Common.sEquals
								+ Common.sDoubleQuotation + unitNameArray[i]
								+ Common.sDoubleQuotation 
								+ Common.sRightParenthesis + Common.sRightParenthesis );
					}else{
				...
				sQuery.append(Common.sCRLF + Common.sAnd + tmpQuery.toString());
			}else if(!tranVo.getFromUnit().contains(Common.sUnderbar)&&tranVo.getFromUnit().contains(Common.sMinus)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getFromUnit().split(Common.sMinus);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
				...
				sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
						+ Common.sSOURCEUNITNAME + Common.sEquals
						+ Common.sDoubleQuotation + tranVo.getFromUnit() 
						+ Common.sDoubleQuotation + Common.sRightParenthesis);
			}
		}
		
		//toUnit
		if (tranVo.getToUnit()!=null && !tranVo.getToUnit().equals("")) {
			if(tranVo.getToUnit().contains(Common.sUnderbar)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getToUnit().split(Common.sUnderbar);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
								+ Common.sLeftParenthesis + Common.sDESTUNITNAME + Common.sEquals
								+ Common.sDoubleQuotation + unitNameArray[i]
								+ Common.sDoubleQuotation 
								+ Common.sRightParenthesis + Common.sAnd);
					}else if(i==(unitNameArray.length-1)){
						tmpQuery.append(
								Common.sLeftParenthesis
								+ Common.sDESTUNITNAME + Common.sEquals
								+ Common.sDoubleQuotation + unitNameArray[i]
								+ Common.sDoubleQuotation 
								+ Common.sRightParenthesis + Common.sRightParenthesis );
					}else{
				...
				sQuery.append(Common.sCRLF + Common.sAnd + tmpQuery.toString());
			}else if(!tranVo.getToUnit().contains(Common.sUnderbar)&&tranVo.getToUnit().contains(Common.sMinus)){
				StringBuilder tmpQuery = new StringBuilder();
				String[] unitNameArray = tranVo.getToUnit().split(Common.sMinus);
				for(int i=0;i<unitNameArray.length;i++){
					if(i==0){
						tmpQuery.append(
								Common.sCRLF + Common.sLeftParenthesis
				...
				sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
						+ Common.sDESTUNITNAME + Common.sEquals
						+ Common.sDoubleQuotation + tranVo.getToUnit() 
						+ Common.sDoubleQuotation + Common.sRightParenthesis);
			}
		}
		...
		// 2021.03.22	X0122410 : 기존 TRANSPORTTYPE > TRANSPORTTYPE2
		// sTransportTYPE
		/*
		if (tranVo.getTransportMachineType()!=null && tranVo.getTransportMachineType().size() > 0) {
			List<String> typeList = tranVo.getTransportMachineType();
			...
			for (int i=0;i<typeList.size();i++) {
				...
				if(i==0) {
					sQuery.append(Common.sLeftParenthesis +
						Common.sTRANSPORTTYPE + Common.sEquals +
...
//				+ Common.sCRLF + Common.sFields + Common.sTRANSPORTCOMMANDID + Common.sComma + Common.sTRANSPORTJOBID + Common.sComma
//				+ Common.sSTATE + Common.sComma + Common.sCARRIER + Common.sComma + Common.sPRIORITY + Common.sComma
//				+ Common.sDESCRIPTION + Common.sComma + Common.sComma + Common.sFIXEDROUTE
//				+ Common.sComma + Common.sSOURCEMACHINENAME + Common.sComma + Common.sSOURCEUNITNAME + Common.sComma 
...
//				+ Common.sDESTTYPE + Common.sComma + Common.sDESTUNITNAME + Common.sComma + Common.sTIME_EX
//				+ Common.sComma + Common.sTRANSPORTMACHINENAME + Common.sComma + Common.sTRANSPORTUNITNAME );	
//		sQuery.append(Common.sFrom + sTable);
...
//		sQuery.append(Common.sCRLF + tQueryToFab.toString());
		...
		sQuery.append(Common.sCRLF
				+ Common.sFields + Common.sTRANSPORTCOMMANDID + Common.sComma + Common.sTRANSPORTJOBID + Common.sComma
				+ Common.sSTATE + Common.sComma + Common.sCARRIER + Common.sComma + Common.sPRIORITY + Common.sComma
				+ Common.sDESCRIPTION + Common.sComma + Common.sComma + Common.sFIXEDROUTE
				+ Common.sComma + Common.sSOURCEMACHINENAME + Common.sComma + Common.sSOURCEUNITNAME + Common.sComma 
				+ Common.sSOURCEAREANAME + Common.sComma + Common.sSOURCEBAYNAME + Common.sComma
				+ Common.sSOURCEMACHINETYPE + Common.sComma + Common.sDESTMACHINENAME + Common.sComma
				+ Common.sDESTAREANAME + Common.sComma + Common.sDESTBAYNAME + Common.sComma
				+ Common.sDESTTYPE + Common.sComma + Common.sDESTTYPE2 + Common.sComma + Common.sDESTUNITNAME + Common.sComma + Common.sTIME_EX
				+ Common.sComma + Common.sTRANSPORTMACHINENAME + Common.sComma + Common.sTRANSPORTTYPE2 + Common.sComma + Common.sTRANSPORTUNITNAME
				);
