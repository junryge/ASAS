 ...
 * @FileName   : TranReturnJobController.java
 * @작성일        : 2017. 3. 20. 
 * @작성자        :  박민호
 * @프로그램 설명 : 반송 JOB 이력 조회 컨트롤러
 */
@Controller
public class TranJobHistoryController {
	protected Log log = LogFactory.getLog(TranJobHistoryController.class);
	@Resource(name = "tranJobHistoryService")
	private TranService tranService;
	
//	@Resource(name = "totalService")
//	private TotalService totService;
	
	@RequestMapping(value = "tran/returnJobLogList")
...
//		param.setToFab(Common.getBasicFabList("tran",Common.sFAB_SITE));
		...
		mav.setViewName("tran/returnJobLogList");
		return mav;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "tran/ajax/getReturnJobLogList")
	public ModelAndView getReturnJobLogList(@ModelAttribute TranVo param, HttpServletRequest request) throws Exception {
		log.info("getReturnJobLogList : Start!!");
		ModelAndView mav = new ModelAndView();
