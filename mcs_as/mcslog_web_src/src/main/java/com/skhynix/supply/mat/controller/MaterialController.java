 ...
 * 
 */
package com.skhynix.supply.mat.controller;
...
/**
 * @Package Name : com.skhynix.supply.mat.controller
 * @FileName : MaterialController.java
 * @작성일 : 2017. 3. 16.
 * @작성자 : 박민호
 * @프로그램 설명 : Material 로그 조회
 */
@Controller
public class MaterialController {
	protected Log log = LogFactory.getLog(MaterialController.class);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "mat/carrierLocLogList")
	public ModelAndView carrierLocLogList(@ModelAttribute MaterialVo param, HttpServletRequest request) 	throws Exception {
...
//		param.setFabSite(sFabSite);
		...
		//fab
		mav.addObject("fabs", Common.getFabList("mat", sFabSite));
		param.setFab(Common.getBasicFabList("mat", sFabSite));
		//level
...
//		mav.setViewName("alarm/alarmReportLogList");
		mav.setViewName("mat/carrierLocLogList");
		return mav;
	}
	
	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/mat/ajax/getCarrierLocLogList.do")
	public ModelAndView getList(@ModelAttribute MaterialVo param, HttpServletRequest request) throws Exception {
		ModelAndView mav = new ModelAndView();
...
//		param.setFabSite(sFabSite);
		...
		//List<String> fabList = Common.getFabList("mat", Common.sFAB_SITE);		
		List<String> fabList = Common.getFabList("mat", sFabSite);
...
//		}
		...
		List list = materialService.getDataList(param);
