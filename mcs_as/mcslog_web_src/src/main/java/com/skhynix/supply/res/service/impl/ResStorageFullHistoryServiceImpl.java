...
@Service("resStorageFullHistoryServiceImpl")
public class ResStorageFullHistoryServiceImpl implements ResHistoryService {
	protected Log log = LogFactory.getLog(ResStorageFullHistoryServiceImpl.class);
	...
	public List<Map> getDataList(ResStorageFullVo resStorageFullVo) throws Exception {
		...
		long offset 	= (Long.parseLong(resStorageFullVo.getPageNum()) - 1) * Long.parseLong(resStorageFullVo.getRowNum());
		int   limit 	= Integer.parseInt(resStorageFullVo.getRowNum());
		String resultQuery = getQueryParser(resStorageFullVo);
		if (resultQuery != null && !(resultQuery.isEmpty())) {
			...
			dataList = Client.dbExecuteQuery(resStorageFullVo.getFabSite(), resultQuery);
		}
		return dataList;
	}
	...
	public String getQueryParser(ResStorageFullVo resStorageFullVo) {
		if (resStorageFullVo == null) {
			return null;
		} // null exception

		StringBuilder sQuery = new StringBuilder();
		sQuery.append(String.format(Common.sFulltext_Arg0_key1, 
				resStorageFullVo.getFrom(), resStorageFullVo.getTo(),
				(Common.sLeftParenthesis + Common.sMETHOD + Common.sEquals
					+ Common.sDoubleQuotation + "createStorageFullHistory"
					+ Common.sDoubleQuotation + Common.sRightParenthesis)));

		// Condition & Filter --> AND , OR
		// State
		if (resStorageFullVo.getState() != null && !resStorageFullVo.getState().equals("")) {
			sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
					+ Common.sSTATE + Common.sEquals
					+ Common.sDoubleQuotation + resStorageFullVo.getState() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		}
		// FullState
		if (resStorageFullVo.getFullState() != null && !resStorageFullVo.getFullState().equals("")) {
			sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
					+ Common.sFULLSTATE + Common.sEquals
					+ Common.sDoubleQuotation + resStorageFullVo.getFullState() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		}
		// ProcessingState
		if (resStorageFullVo.getProcessingState() != null && !resStorageFullVo.getProcessingState().equals("")) {
			sQuery.append(Common.sCRLF+ Common.sAnd + Common.sLeftParenthesis
					+ Common.sPROCESSINGSTATE + Common.sEquals
					+ Common.sDoubleQuotation + resStorageFullVo.getProcessingState() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		}
		
		// sAREANAME
		if (resStorageFullVo.getAreaName()!=null && resStorageFullVo.getAreaName().indexOf(Common.sALL) >= 0) {
		} else {
			sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
					+ Common.sAREANAME + Common.sEquals
					+ Common.sDoubleQuotation + resStorageFullVo.getAreaName() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		} 

		// sBAYNAME
		if (resStorageFullVo.getBayName()!=null && resStorageFullVo.getBayName().indexOf(Common.sALL) >= 0) {
		} else {
			sQuery.append(Common.sCRLF + Common.sAnd + Common.sLeftParenthesis
					+ Common.sBAYNAME + Common.sEquals
					+ Common.sDoubleQuotation + resStorageFullVo.getBayName() 
					+ Common.sDoubleQuotation + Common.sRightParenthesis);
		}
		...
		// sTYPE
		/* 
		if (resStorageFullVo.getMachineType()!=null && resStorageFullVo.getMachineType().size() > 0) {
			List<String> typeList = resStorageFullVo.getMachineType();
		...
		*/ 
		...
		if (resStorageFullVo.getMachineType()!=null && resStorageFullVo.getMachineType().size() > 0) {			
			...
			for (String s : resStorageFullVo.getMachineType()) {
		...
		// sMACHINENAME
		if (resStorageFullVo.getMachineName()!=null && resStorageFullVo.getMachineName().size() > 0) {
			List<String> machineList = resStorageFullVo.getMachineName();
...
//		String sTable = getTableFromFab();
		...
		if (resStorageFullVo.getFab()!=null && resStorageFullVo.getFab().size() > 0) {
			List<String> fab = resStorageFullVo.getFab();
			String fabName = "";
			log.info("fab:"+resStorageFullVo.getFab());
			
			for(int i=0;i<resStorageFullVo.getFab().size();i++){
				//2022. 6.15. X0122410 : fab site 접근로직 변경 
				//fabName = getTableFromFab(Common.sFAB_SITE,fab.get(i));
				fabName = getTableFromFab(resStorageFullVo.getFabSite(),fab.get(i));
				if(fabName != null && !fabName.isEmpty()) {
					if(i==0){
						tQuery.append(fabName);
					}else if(i==(resStorageFullVo.getFab().size()-1)) {
...
//				+ Common.sFields + Common.sTIME_EX + Common.sComma + Common.s_TIME + Common.sComma 
//				+ Common.sMACHINENAME + Common.sComma + Common.sSTATE + Common.sComma
//				+ Common.sPROCESSINGSTATE + Common.sComma + Common.sFULLSTATE
//				); 
		...
		sQuery.append(Common.sCRLF
				+ Common.sFields + Common.sTIME_EX + Common.sComma + Common.s_TIME + Common.sComma 
				+ Common.sMACHINENAME + Common.sComma + Common.sMACHINETYPE + Common.sComma + Common.sSTATE + Common.sComma
				+ Common.sPROCESSINGSTATE + Common.sComma + Common.sFULLSTATE);
