 ...
 */
@Controller
public class ResMachineHistoryController {
	protected Log log = LogFactory.getLog(ResMachineHistoryController.class);
	@Resource(name = "resMachineHistoryServiceImpl")
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "res/machineLogList")
	public ModelAndView carrierLocLogList(@ModelAttribute ResMachineVo param, HttpServletRequest request) throws Exception {
...
//		param.setFabSite(sFabSite);
		...
		mav.setViewName("res/machineLogList");
		return mav;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "res/ajax/getMachineLogList")
	public ModelAndView getMachineLogList(@ModelAttribute ResMachineVo param, HttpServletRequest request) throws Exception {
		ModelAndView mav = new ModelAndView();
...
//		}
		...
		// 변수 초기화 (level)
		/*
		 * List<String> levels = new ArrayList<String>(); for (int i = 1; i <=
		 * 8; i++) { String result = request.getParameter("level" + i); if
		 * (result != null && !result.equals("")) { levels.add(result); } }
		 * param.setLevel(levels);
		 */
