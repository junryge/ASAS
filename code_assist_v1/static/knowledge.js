/* code_assist_v1/static/knowledge.js — 도메인 지식 (단일 폴더, user_id 없음) */

const Knowledge = {
  async fetchList() {
    return await api("/api/code/knowledge/list");
  },
  ...
  async openView(filename) {
    try {
      const data = await api(`/api/code/knowledge/view/${encodeURIComponent(filename)}`);
      ...
      Modal.open({
        ...
        footButtons: [
          { label: "삭제", danger: true, onClick: async () => {
            ...
            await api("/api/code/knowledge/delete", { method: "POST", body: { filename } });
            toast("삭제됨", "ok");
            Knowledge.render();
          }},
          { label: "저장", primary: true, onClick: async () => {
            await api("/api/code/knowledge/create", {
              method: "POST",
              body: { filename, content: ta.value },
            });
            toast("저장됨", "ok");
            Knowledge.render();
          }},
