...
def _resolve_model(model_id: str | None) -> tuple[dict | None, str]:
    ...
    # API: model_id 는 registry key 또는 env_id
...
def _stream_gguf(
    ...
    """GGUF 모델 SSE 스트림 (자체 utils.gguf_model 사용 — demos_v1 와 독립)."""
    import code_assist_v1.utils as _u
    if _u.gguf_model is None:
        yield _sse({"error": "GGUF 모델이 로드되지 않았습니다 (MODEL_GGUF/ 에 파일을 넣고 재시작)", "status": 503})
        return
    try:
