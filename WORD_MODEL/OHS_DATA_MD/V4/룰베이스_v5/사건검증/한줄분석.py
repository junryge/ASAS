...
def analyze(r):
    ...
    for a in AREAS5:
        ...
        if f(r,f'{a}_pts_RA')>0 or f(r,f'{a}_pts_RA_sus')>0:
            v=f(r,f'{a}_ra'); lines.append(f"반송지연  {RA_COL[a]} = {v:.2f}분 (임계 {TH_RA[a]})")
        if f(r,f'{a}_pts_RB')>0 or f(r,f'{a}_pts_RB_fast')>0:
            v=f(r,f'{a}_rb_diff30'); lines.append(f"큐누적    {RB_COL[a]} = +{int(v)}/30분 (임계 {TH_RB[a]})")
        if f(r,f'{a}_pts_RC')>0 and a=='M16HUB':
            v=f(r,'M16HUB_rev_count'); lines.append(f"리프터역증가  역증가 {int(v)}개 (임계 {TH_RC}) [{r.get('M16HUB_rev_lids','')}]")
        if f(r,f'{a}_pts_RD')>0:
            if a=='M16HUB':
                lines.append(f"FAB/STB   FABSTORAGERATIO={f(r,'M16HUB_rd_fab'):.1f}% / STB.3F_STORAGE_UTIL={f(r,'M16HUB_stb_util'):.1f}% (임계 FAB{TH_FAB}/STB{TH_STB})")
            else:
                v=f(r,f'{a}_rd_oht'); lines.append(f"OHT정체   {OHT_COL[a]} = {v:.1f}% (임계 {TH_OHT})")
        if f(r,f'{a}_pts_SLA')>0:
            v=f(r,f'sla_{a}'); lines.append(f"SLA초과   {SLA_COL[a]} = {v:.1f}% (임계 {TH_SLA.get(a,'-')})")
        if f(r,f'{a}_pts_SORT')>0:
            v=f(r,f'sorter_{a}'); lines.append(f"Sorter    {SORT_COL[a]} = {int(v)}LOT 대기")
        if f(r,f'{a}_pts_MAXCAPA')>0:
            lines.append(f"MAXCAPA   {a}.QUE.CNV.3F_CNV_MAXCAPA 변경")
        if lines:
            print(f"\n   ▶ {a}")
