...
class ReplayEngine:
    ...
    def load_date(self, date_key: str, date_config: Optional[dict] = None) -> dict:
        """특정 날짜 데이터 로드. date_config 미지정 시 기본 DATA_DATES 조회."""
        self.state = ReplayState.STOPPED
    ...
    def get_current_snapshot(self) -> dict:
        ...
        return {
            ...
            # 실시간 데드락 핫스팟 (주소 클러스터 / 체인)
            'hotspots': self.world.get_deadlock_hotspots(self.layout),
        }
    ...
    def get_hotspot_history(self, sample_interval_sec: int = 30) -> List[dict]:
        """
        전체 OHT 타임라인을 스캔해 핫스팟 이벤트 이력 생성.
        누적 차량 상태를 시간 흐름대로 따라가며 sample_interval_sec 간격으로
        주소 클러스터/체인 데드락 감지 → 시작/피크/종료 이벤트로 집계.
        """
        if not self.data_loader or not self.data_loader.oht_timeline:
            return []
        ...
        def make_key(kind: str, addrs: List[int]) -> str:
            lo, hi = min(addrs), max(addrs)
            return f"{kind}:{lo}-{hi}"

        def scan_hotspots(t) -> List[dict]:
            """현재 누적 상태에서 핫스팟 탐지 (world_model.get_deadlock_hotspots와 동일 로직)"""
            addr_stopped = defaultdict(list)
            for v in cumulative.values():
                st = v.get('state')
                a = v.get('currentNode', 0)
                if st in stopped_states and a > 0:
                    addr_stopped[a].append(v)

            out = []
            # 단일 주소 3대+
            for a, vs in addr_stopped.items():
                if len(vs) < 3:
                    continue
                sb = defaultdict(int)
                for v in vs:
                    sb[str(v.get('state'))] += 1
                sev = 'CRITICAL' if len(vs) >= 8 else 'DANGER' if len(vs) >= 5 else 'WARN'
                out.append({'kind': 'cluster', 'addrs': [a], 'stopped': len(vs), 'severity': sev, 'state_breakdown': dict(sb)})
            # 체인 (±3 인접, 3주소+ 총 5대+)
            seeds = sorted(a for a, vs in addr_stopped.items() if len(vs) >= 2)
            visited = set()
            for a in seeds:
                if a in visited: continue
                chain = [a]; visited.add(a)
                for nb in seeds:
                    if nb in visited: continue
                    if any(abs(nb - c) <= 3 for c in chain):
                        chain.append(nb); visited.add(nb)
                if len(chain) < 3: continue
                total = sum(len(addr_stopped[c]) for c in chain)
                if total < 5: continue
                sb = defaultdict(int)
                for c in chain:
                    for v in addr_stopped[c]: sb[str(v.get('state'))] += 1
                sev = 'CRITICAL' if total >= 15 else 'DANGER' if total >= 10 else 'WARN'
                out.append({'kind': 'chain', 'addrs': sorted(chain), 'stopped': total, 'severity': sev, 'state_breakdown': dict(sb)})
            return out

        sev_rank = {'WARN': 1, 'DANGER': 2, 'CRITICAL': 3}
        last_sample_t = None
        time_start = self.data_loader.time_start
        ...
        for t, updates in self.data_loader.oht_timeline:
            ...
            for v in updates:
                ...
                if vid:
                    cumulative[vid] = v
            # 샘플링 간격 미달이면 건너뜀
            if last_sample_t is not None and (t - last_sample_t).total_seconds() < sample_interval_sec:
                continue
            last_sample_t = t

            hotspots = scan_hotspots(t)
            t_str = t.strftime("%H:%M:%S")
            current_keys = set()

            for hs in hotspots:
                # WARN 이상만 이력 남김 (노이즈 제거)
                if sev_rank.get(hs['severity'], 0) < 1:
                    continue
                key = make_key(hs['kind'], hs['addrs'])
                current_keys.add(key)
                if key in active:
                    ev = active[key]
                    ev['end'] = t_str
                    if hs['stopped'] > ev['peak']:
                        ev['peak'] = hs['stopped']
                        ev['peakTime'] = t_str
                    if sev_rank.get(hs['severity'], 0) > sev_rank.get(ev['maxSev'], 0):
                        ev['maxSev'] = hs['severity']
                else:
                    # 좌표 중심
                    cx = cy = None
                    if self.layout:
                        xs, ys = [], []
                        for a in hs['addrs']:
                            if a in self.layout.nodes:
                                xs.append(self.layout.nodes[a][0])
                                ys.append(self.layout.nodes[a][1])
                        if xs:
        ...
        # 최근순 정렬 (시작 시각 내림차순)
        finished.sort(key=lambda e: e['start'], reverse=True)
        return finished
