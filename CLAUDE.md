...
### 아이디별 리포트 등록/다운로드 (2026-07 추가)

- 관리자 "레포트 등록 완료" = PUT /api/reports/{날짜}?dist=1 → 서버가 모든 운영담당자
  아이디별로 같은 body 를 각자 사본(opBodies)으로 즉시 등록 (재등록 시 전원 초기화, 클라이언트에서 confirm)
- 관리자 뷰어에서 원본 저장(쿼리 없음)은 사본 보존 — 배포는 등록 버튼(dist=1)만
- 목록(관리자): "다운로드 대상: [등록 원본][op1][op2]…" 필 선택 후 기간 HTML/MD 다운로드
  (파일명 뒤에 _아이디 붙음, 대상 사본 없으면 원본으로 대체)

### 등록 체크박스/완료 표시/삭제 전파 (2026-07 추가)

- 레포트 등록 화면(관리자)에 "등록 대상 아이디" 체크박스: 전체(ALL) + 아이디별
  - 전체 체크 = PUT ?dist=1 (전원 배포) / 일부 체크 = 대상별 PUT ?op=아이디 (빈 날짜면 서버가 원본도 생성)
- 운영담당자 뷰어에 "✓ 레포트 등록 완료" 버튼 → POST /api/reports/{날짜}/complete
  (opBodies[아이디].status='completed', 내용 재저장해도 완료상태 유지, 관리자 재등록 시 초기화)
