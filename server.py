...
# -*- coding: utf-8 -*-
"""
AMHS Log Analysis Server (v5.0 - 배치 처리 지원)
        ...
        try:
            ...
            if csv_text is None:
                results.append({
                    ...
                    "success": False,
                    "error": "CSV 파일 인코딩 실패 (UTF-8, CP949, EUC-KR 모두 실패)",
                    "order": idx + 1
                })
                continue
            ...
            logger.info(f"  ✅ {file.filename}: {result.get('equipment_type', 'UNKNOWN')}, "
                       f"{result.get('basic_info', {}).get('row_count', 0)} rows")
            
        except Exception as e:
            logger.error(f"  ❌ {file.filename} failed: {e}")
            results.append({
                "filename": file.filename,
                "success": False,
                "error": str(e),
...
async def analyze_poi_batch(data: dict):
    ...
    for idx, filename in enumerate(filenames):
        ...
        try:
            ...
            logger.info(f"  ✅ {filename}: {result.get('equipment_type', 'UNKNOWN')}")
