...
class RealOHTParser:
    ...
    @classmethod
    def from_server(cls, server_url: str) -> 'RealOHTParser':
        """
        sim_server에서 레이아웃 데이터를 받아 파서 초기화 (XML 로딩 불필요).
        sim_server가 먼저 실행되어 있어야 함.

        Args:
            server_url: sim_server 주소 (예: "http://localhost:8000")
        """
        ...
        print(f"sim_server에서 레이아웃 로딩: {server_url}/api/layout")
        with urllib.request.urlopen(f"{server_url}/api/layout", timeout=10) as resp:
            data = json.loads(resp.read())

        for n in data.get('nodes', []):
            obj.nodes[n['no']] = {'no': n['no'], 'x': n['x'], 'y': n['y']}

        for e in data.get('edges', []):
            from_n, to_n = e['from'], e['to']
            obj.connections.append([from_n, to_n])
            if from_n in obj.nodes and to_n in obj.nodes:
                n1, n2 = obj.nodes[from_n], obj.nodes[to_n]
                dist = math.sqrt((n2['x'] - n1['x'])**2 + (n2['y'] - n1['y'])**2)
                obj.edge_dist[(from_n, to_n)] = dist

        print(f"서버에서 로딩 완료: {len(obj.nodes)}개 노드, {len(obj.edge_dist)}개 엣지")
        return obj
    ...
    # ============================================================
    # 차량 이동 시뮬레이션 (sim_server와 연동용)
    # ============================================================

    def init_vehicles(self, messages: List[str]):
        """
        메시지 리스트로 차량 이동 상태 초기화.
        이후 update_vehicles()를 반복 호출하면 트랙을 따라 이동.

        Args:
            messages: VHL_STATE_REPORT 메시지 리스트 (SAMPLE_MESSAGES 등)
        """
        # 인접 노드 맵 빌드 {from_addr: [to_addr, ...]}
        self._adj: Dict[int, List[int]] = {}
        for conn in self.connections:
            from_n, to_n = conn[0], conn[1]
            if from_n not in self._adj:
                self._adj[from_n] = []
            self._adj[from_n].append(to_n)

        # 차량별 이동 상태 초기화
        self._movers: Dict[str, dict] = {}
        for r in self.parse_messages(messages):
            edge_d = self.edge_dist.get((r['currentAddress'], r['nextAddress']), 1.0)
            init_ratio = r['distance_mm'] / (edge_d * 1000) if edge_d > 0 else 0.0
            self._movers[r['vehicleId']] = {
                'vehicleId':    r['vehicleId'],
                'currentAddr':  r['currentAddress'],
                'nextAddr':     r['nextAddress'],
                'ratio':        min(1.0, init_ratio),
                'x':            r['x'],
                'y':            r['y'],
                'velocity':     180.0,             # m/min 기본 속도
                'state':        int(r['state']),
                'stateName':    r['stateName'],
                'isLoaded':     r['isFull'],
                'carrierId':    r['carrierId'],
        ...
        print(f"차량 이동 초기화: {len(self._movers)}대")

    def update_vehicles(self, dt: float) -> List[dict]:
        """
        dt초 동안 각 차량을 트랙 위에서 이동시키고
        WebSocket 전송용 vehicle 리스트를 반환.

        Args:
            dt: 경과 시간 (초), 0이면 이동 없이 현재 상태만 반환
        """
        for m in self._movers.values():
            ca = m['currentAddr']
            na = m['nextAddr']

            # 현재 엣지 거리 (좌표 단위, ×1000 = mm)
            edge_dist = self.edge_dist.get((ca, na), 0.0)
            if edge_dist <= 0:
                # 연결 없으면 다음 노드로 교체
                neighbors = self._adj.get(ca, [])
                if neighbors:
                    na = neighbors[0]
                    m['nextAddr'] = na
                    edge_dist = self.edge_dist.get((ca, na), 1.0)
                else:
                    continue

            if dt > 0:
                # 엔진과 동일한 스케일: move(mm) / edge_dist(좌표단위)
                move_mm = m['velocity'] * (dt / 60.0) * 1000
                m['ratio'] += move_mm / max(edge_dist, 1.0)

            # 다음 노드 도달 → 랜덤 방향으로 전환
            if m['ratio'] >= 1.0:
                m['ratio'] = 0.0
                m['currentAddr'] = na
                neighbors = self._adj.get(na, [])
                m['nextAddr'] = random.choice(neighbors) if neighbors else na

            # 좌표 보간
            n1 = self.nodes.get(m['currentAddr'])
            n2 = self.nodes.get(m['nextAddr'])
            if n1 and n2:
                r = m['ratio']
                m['x'] = round(n1['x'] + (n2['x'] - n1['x']) * r, 2)
                m['y'] = round(n1['y'] + (n2['y'] - n1['y']) * r, 2)
            elif n1:
                m['x'] = n1['x']
                m['y'] = n1['y']
        ...
        return [{
            ...
            'isLoaded':     m['isLoaded'],
            'velocity':     m['velocity'],
            'currentNode':  m['currentAddr'],
            'nextNode':     m['nextAddr'],
            'destination':  m['destination'],
            'carrierId':    m['carrierId'],
            'path':         [],
            'runCycle':     m['runCycle'],
            'runCycleName': m['runCycleName'],
            'vhlCycle':     m['vhlCycle'],
            'vhlCycleName': m['vhlCycleName'],
            'distance':     round(m['ratio'] * self.edge_dist.get(
                                (m['currentAddr'], m['nextAddr']), 1.0) * 1000, 2),
            'detailState':  m['detailState'],
...
# ============================================================
# 샘플 데이터 (실제 OHT 메시지 예시) - import 가능하도록 모듈 레벨 정의
# ============================================================
SAMPLE_MESSAGES = [
...
def main():
    ...
    # sim_server에서 레이아웃 받기 시도 (빠름) → 실패 시 로컬 XML 사용
    try:
        fast_parser = RealOHTParser.from_server(SERVER_URL)
        print("sim_server 레이아웃 사용 (빠른 시작)")
    except Exception:
        print("sim_server 연결 실패 - 로컬 레이아웃 사용")
        fast_parser = parser  # 이미 로딩된 parser 재사용

    fast_parser.init_vehicles(SAMPLE_MESSAGES)

    print(f"실시간 이동 모드 시작 → {SERVER_URL} 으로 전송")
    print("Ctrl+C 로 종료")
    print("=" * 70)
    ...
    try:
        while True:
            vehicles = fast_parser.update_vehicles(INTERVAL)
            tick += 1

            payload = json.dumps({"vehicles": vehicles}).encode('utf-8')
            req = urllib.request.Request(
                f"{SERVER_URL}/api/real-vehicles",
                data=payload,
            ...
            try:
                ...
                if tick % 10 == 0:
                    v0 = vehicles[0]
                    print(f"[tick {tick:4d}] {v0['vehicleId']} "
                          f"node={v0['currentNode']} ({v0['x']:.1f}, {v0['y']:.1f})")
            except urllib.error.URLError:
                print(f"[주의] sim_server({SERVER_URL}) 연결 실패. "
                      f"sim_server_3d_D5_D7.py 를 먼저 실행하세요.")
                time.sleep(2)
                continue
