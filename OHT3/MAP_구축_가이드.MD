...
내장된 샘플 데이터 5건을 파싱하여 결과를 출력하고, CSV/JSON 파일을 `output/` 폴더에 저장한다.
...
```
data_sender.py (또는 실제 OHT 시스템)
    │
    │  POST /api/oht-raw
    │  {"messages": ["2,OHT,V00795,...", ...]}
    ▼
sim_server_3d_D5_D7.py (포트 10003)
    │  └── real_oht_parser.py (import)
    │       └── parse_messages() → 메시지 파싱
    │       └── to_map_json()   → x,y 좌표 변환
    │
    │  WebSocket (/ws) - 0.5초 간격
    ▼
브라우저 MAP UI (http://localhost:10003)
```
...
| 파일 | 역할 |
|------|------|
| `sim_server_3d_D5_D7.py` | MAP UI 서버. WebSocket으로 차량 상태 전송 |
| `real_oht_parser.py` | OHT 메시지 파싱 + layout.xml 좌표 변환 (sim_server가 import) |
| `data_sender.py` | OHT 메시지를 sim_server에 전송 (테스트용 / 실데이터 연결용) |
| `layout_map_cre.py` | layout.xml → layout.html 변환 |
| `hid_zone_csv_cre.py` | HID Zone 마스터 CSV 생성 |
...
| 엔드포인트 | 메서드 | 설명 |
|------------|--------|------|
| `/api/oht-raw` | POST | raw OHT 메시지 수신 → 파싱 → 차량 위치 업데이트 |
| `/api/real-vehicles` | POST | 파싱된 차량 dict 리스트 직접 수신 |
| `/api/layout` | GET | 레이아웃 데이터 (노드, 엣지) 반환 |
| `/ws` | WebSocket | 실시간 차량 상태 전송 (0.5초 간격) |

### 8.4 CSV 출력

`output/{FAB명}/{prefix}/` 폴더에 저장:
- `ATLAS_VEHICLE_{timestamp}.csv` - 차량 상태 기록
- `ATLAS_RAIL_TRAFFIC_{timestamp}.csv` - 레일 통행량
...
```
- 포트: 10003
- 시작 시 차량 0대 (data_sender가 보내기 전까지 빈 화면)
...
```
- `1` 선택: 자동 모드 (sim_server에서 레이아웃 받아 5대 자동 이동)
- `2` 선택: 붙여넣기 모드 (OHT 메시지 직접 입력)

### 9.3 실제 OHT 연동

실제 시스템에서 OHT 메시지를 받으면 `/api/oht-raw` 로 POST:
...
requests.post("http://localhost:10003/api/oht-raw", json={"messages": messages})
```

0.5초 간격으로 전송하면 MAP UI에서 실시간 차량 이동이 표시된다.
